...
\mode<article>{
This lecture examines privacy's philosophical foundations and practical challenges in the information age. We begin by defining privacy and explaining why it matters, both instrumentally (as means to other goods like autonomy and democracy) and intrinsically (as constitutive of personhood and dignity). Information technology has transformed privacy by enabling unprecedented surveillance, data collection, and processing. The history of privacy debates around successive technologies---from census-taking through telephone wiretaps to internet tracking---reveals recurring patterns and tensions. We consider how surveillance operates in authoritarian states versus liberal democracies, and whether these contexts present fundamentally different ethical questions. The lecture concludes by evaluating privacy protection strategies and considering privacy's status as a universal value versus culturally contingent preference.
}

% Slide 1: Title
\lecturetitleslide{images/fractal_06.png}


% ====================
% COLD OPEN
% ====================
% Again the data is on their own devices, so it cannot go stale.

\begin{frame}{Before We Begin: Open Your Settings}
\textbf{Guess first.} How many apps on your phone are currently allowed to see your \textbf{location}? How many can use your \textbf{microphone}?

\vspace{0.2em}
Now count them:
\begin{itemize}
    \item iPhone: \emph{Settings $\rightarrow$ Privacy \& Security $\rightarrow$ Location Services}
    \item Android: \emph{Settings $\rightarrow$ Location $\rightarrow$ App location permissions}
\end{itemize}

\begin{alertblock}{The interesting number is not the count}
It is how many you \textbf{would not have been able to name} before you looked --- and how many of those you agreed to on purpose, in a moment you can actually remember.
\end{alertblock}

\begin{discussionbox}[Commit before we go on]
Whose guess was furthest off? Was the class biased \textbf{high or low} --- and what does a systematic error in one direction tell us about how the consent was obtained?
\end{discussionbox}
\end{frame}
...
\textbf{Lecture 1 (History)}: The telegraph, telephone, and early internet each prompted new surveillance debates. Privacy conflicts trace the same liberation-then-control pattern as every other IT revolution.

\vspace{0.2cm}
\textbf{Lecture 2 (Virtues)}: Surveillance changes behavior even when unobserved---the \textit{chilling effect}. Does constant monitoring undermine the intellectual and moral virtues Aristotle valued?

\vspace{0.2cm}
\textbf{Lecture 4 (IP)}: Data as a resource: platforms collect personal data to personalize ads and content. How does data collection relate to IP, ownership, and the enshittification dynamic?

\vspace{0.2cm}
\textbf{Lecture 5 (Crypto)}: Cryptocurrency was partly designed to protect financial privacy. Is financial surveillance relevantly different from communications surveillance?

\begin{discussionbox}[Discussion]
Is privacy a universal human value, or is it culturally contingent? Could you make the case either way?
\end{discussionbox}
\end{frame}
...
\mode<article>{
Privacy proves surprisingly difficult to define precisely despite its intuitive importance. Early formulations like Warren and Brandeis's ``right to be let alone'' \parencite{warren_brandeis_privacy} and information-control theories face challenges around boundary cases. Helen Nissenbaum's contextual integrity framework provides a more nuanced approach: privacy violations occur when information flows in ways inappropriate to social contexts \parencite{nissenbaum_privacy_context}. Privacy encompasses multiple dimensions---informational, physical, decisional, communicational, associational, and intellectual---each potentially requiring different protections. Instrumental arguments for privacy emphasize its functional importance for autonomy, intimacy, democracy, security, and power balance. Intrinsic arguments contend privacy has value beyond consequences as a requirement of human dignity and respect for persons as ends in themselves.
}
...
\begin{itemize}
    \item ``The right to be let alone'' \parencite{warren_brandeis_privacy}
    \item Control over information about oneself
    \item \textbf{Contextual integrity} (Helen Nissenbaum) \parencite{nissenbaum_privacy_context}: Information flows appropriate to context
\end{itemize}
\end{conceptbox}
...
\vspace{0.3em}
\textbf{James Rachels} \parencite{rachels_privacy}: Privacy enables us to maintain different relationships with different people.
\end{column}
\begin{column}{0.4\textwidth}
\begin{center}
\begin{tikzpicture}[scale=0.55]
    \fill[boxpurple!60] (0,0) circle (2.5);
    \fill[boxpurple!40] (0,0) circle (1.8);
    \fill[boxpurple!20] (0,0) circle (1.1);
...
\begin{enumerate}
    ...
    \item \textbf{Relational}: Privacy constitutes relationships \parencite{rachels_privacy,nissenbaum_privacy_context}
...
\begin{conceptbox}[Judith Jarvis Thomson's Reductionist Challenge \parencite{thomson_privacy}]
\small
...
\begin{objectionbox}[Richard Posner's Economic Argument \parencite{posner_privacy}]
\small
...
\textbf{The most common objection to privacy}: ``If you have nothing to hide, you have nothing to fear.''

\vspace{0.3em}
Two quick problems, to sit with for now:
\begin{itemize}
    \item \textbf{Everyone has something to hide}---not crimes, but legal matters that are nobody else's business. ``Nothing to hide'' quietly equates privacy with guilt.
    \item \textbf{Aggregation}: individually harmless facts combine into a revealing profile. \textcite{solove_understanding_privacy} argues this is where the real damage happens---no single disclosure looks like a violation.
\end{itemize}

\vspace{0.3em}
This argument is important enough to deserve a careful treatment. We will put it in standard form and examine each premise in Part IV, once we have the surveillance cases on the table.
\end{frame}
...
% ===== IMAGE SLIDE: Panopticon =====
\begin{frame}[shrink=8]{The Panopticon: Architecture of Surveillance}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{block}{Jeremy Bentham's Design (1791)}
\begin{itemize}
  \item A circular prison: one guard in a \textbf{central tower} can watch every cell
  \item Prisoners \textbf{cannot tell} when they're being watched
  \item So they \textbf{internalize} the gaze---self-regulating even when unobserved
  \item Foucault used it to analyze \textbf{modern power} in \textit{Discipline and Punish} (1975)
\end{itemize}
\end{block}
\begin{discussionbox}[Discussion]
Is \textbf{social media} a kind of panopticon---where the audience is both the watcher \textit{and} the watched?
\end{discussionbox}
\end{column}
\begin{column}{0.47\textwidth}
\begin{figure}
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{images/panopticon.jpg}
\caption{\scriptsize Panopticon floor plan, from Jeremy Bentham, \textit{Works} (1843). Public domain.}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
    \item \textbf{1890}: Warren \& Brandeis ``The Right to Privacy'' \parencite{warren_brandeis_privacy}---response to photography and tabloid journalism
...
% ===== IMAGE SLIDE: NSA Headquarters =====
\begin{frame}{The NSA: Inside the Surveillance State}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{block}{National Security Agency}
\begin{itemize}
  \item Founded in \textbf{1952}---its very existence was classified for decades
  \item Headquarters at \textbf{Fort Meade, Maryland}; largest employer of mathematicians in the US
  \item Post-9/11: authority dramatically expanded under the \textbf{PATRIOT Act}
  \item Snowden revealed it collected \textbf{bulk phone metadata} on millions of Americans with FISA court approval
  \item Annual budget: \$10--15 billion (classified)
\end{itemize}
\end{block}
\end{column}
\begin{column}{0.47\textwidth}
\begin{figure}
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{images/nsa_headquarters.jpg}
\caption{\scriptsize NSA Headquarters, Fort Meade, Maryland. US Government photo. Public domain.}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\midrule
Market size (est., 2026) & \$300B+ \\
Registered in California & 581 \\
Identified across 5 states & $\sim$750 \\
Attributes per US consumer & 1,500+ \\
\bottomrule
\end{tabular}
\end{table}

\vspace{0.2em}
{\scriptsize Treat the market-size figure carefully: it comes from commercial research reports that disagree with each other by about 20\% and define ``data broker'' loosely. That an industry this large cannot be measured to better than a fifth is itself part of the problem.}
\end{column}
\end{columns}
\end{frame}
...
\begin{conceptbox}[Shoshana Zuboff's Framework \parencite{zuboff_surveillance_capitalism}]
\small
\begin{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Social Credit System}: often portrayed in the West as one unified ``citizen score''---in reality a patchwork of blacklists, local pilots, and regulatory records
    \item \textbf{600--700 million CCTV cameras} nationwide (estimates vary widely)
...
\vspace{0.2em}
\textbf{Social Credit in practice} (Daum; MERICS):
\begin{itemize}
    \item Court \textbf{judgment-defaulter blacklists} do bar named individuals from flights, high-speed rail, and some jobs---but by court order, not an algorithmic score.
    \item Local point-scoring pilots existed; most were scaled back. \textbf{The surveillance is real; the unified ``score'' mostly isn't.}
\end{itemize}
\end{frame}
...
\begin{tikzpicture}[scale=0.8]
    % Balance beam
    \draw[thick, boxblue] (-3,0) -- (3,0);
    \draw[thick, boxblue] (0,-0.5) -- (0,0);
    \fill[boxblue] (0,0) circle (0.1);
    
    % Left side - Security
    \draw[thick, fill=boxgreen!30] (-2.5,-0.5) rectangle (-1,-2);
    \node[font=\small\bfseries] at (-1.75,-1.25) {Security};
    
    % Right side - Liberty
    \draw[thick, fill=boxpurple!30] (1,-0.5) rectangle (2.5,-2);
...
\begin{frame}[shrink=10]{The ``Nothing to Hide'' Argument (Standard Form)}
...
\scriptsize
\begin{itemize}
    \item \textbf{Premise 1 is false}: chilling effects, power imbalances, errors and abuse harm the innocent too.
    \item \textbf{Premise 2 is misleading}: everyone has legal but private matters to hide; the premise conflates privacy with guilt.
    \item \textbf{Ignores aggregation}: innocuous data points combine into sensitive profiles.
    \item \textbf{Ignores context}: what is appropriate in one setting is a violation in another---\textbf{contextual integrity}.
    \item \textbf{Assumes static law}: today's legal behavior may be tomorrow's crime---and who decides what is ``wrong''?
    \item \textbf{Ignores distribution}: surveillance falls hardest on marginalized groups.
\end{itemize}
\end{objectionbox}

{\scriptsize \textbf{Schneier} \parencite{schneier_data_goliath}: ``The real choice is liberty versus control.''}
\end{frame}
...
\begin{frame}[shrink=10]{Case Study: Clearview AI}
\begin{casestudybox}[The Controversial Facial Recognition Company]
\small
\textbf{Database}: Roughly \textbf{50 billion} scraped facial images---up from 3 billion when the company was first exposed in 2020\\
\textbf{Product}: App for police to identify anyone from a photo\\
\textbf{Customers}: 2,400+ law enforcement agencies; federal contracts with ICE, CBP and the Army run through 2026
\end{casestudybox}

\vspace{0.2em}
\textbf{Legal battles}:
\begin{itemize}
    \item Fined \texteuro30.5M (Netherlands), \texteuro20M each (France, Italy, Greece), \pounds7.5M (UK)---and has largely \textbf{not paid}
    \item Banned in Australia and Canada
    \item The Illinois class action settled for a \textbf{23\% equity stake} rather than cash; an appeals court vacated that settlement in 2026
\end{itemize}

\textbf{Company claim}: ``First Amendment right'' to collect public photos
...
\end{frame}
% ===== YOU BE THE JUDGE =====
\begin{frame}[shrink=8]{You Be the Judge: The Attorney General's Options}
You are your state's \textbf{attorney general}. Clearview has scraped tens of billions of images, your residents' faces among them, and sells identification to police. Four options on your desk:

\begin{enumerate}
    \item \textbf{Sue} under state biometric-privacy law and seek to bar the company entirely
    \item \textbf{Negotiate} a consent decree: use permitted, but only for violent felonies, with a warrant
    \item \textbf{License} it for your own state police, on the theory that the database exists either way
    \item \textbf{Nothing} --- this is Congress's problem, not yours
\end{enumerate}

\begin{discussionbox}[Pick one and defend it]
Fines across several countries have already gone largely \textbf{unpaid}. Given that, which option does the most good --- and which sets the \textbf{worst precedent} for the next company that does this?
\end{discussionbox}
\end{frame}
...
% Slide 42: Frameworks for Democratic Surveillance
...
% Slide 43: Key Tensions
\begin{frame}{Privacy in the Balance---Key Tensions}
\begin{center}
\begin{tikzpicture}[scale=0.7, every node/.style={font=\small}]
    % Four tension pairs
    \node[draw, rounded corners, fill=boxblue!20, minimum width=3cm, minimum height=0.8cm] (ind) at (-3,2) {Individual};
    \node[draw, rounded corners, fill=boxgreen!20, minimum width=3cm, minimum height=0.8cm] (col) at (3,2) {Collective};
    \draw[<->, thick] (ind) -- (col);
    
    \node[draw, rounded corners, fill=boxblue!20, minimum width=3cm, minimum height=0.8cm] (pre) at (-3,0.5) {Present};
    \node[draw, rounded corners, fill=boxgreen!20, minimum width=3cm, minimum height=0.8cm] (fut) at (3,0.5) {Future};
    \draw[<->, thick] (pre) -- (fut);
    
    \node[draw, rounded corners, fill=boxblue!20, minimum width=3cm, minimum height=0.8cm] (trans) at (-3,-1) {Transparency};
    \node[draw, rounded corners, fill=boxgreen!20, minimum width=3cm, minimum height=0.8cm] (priv) at (3,-1) {Privacy};
    \draw[<->, thick] (trans) -- (priv);
    
    \node[draw, rounded corners, fill=boxblue!20, minimum width=3cm, minimum height=0.8cm] (inn) at (-3,-2.5) {Innovation};
    \node[draw, rounded corners, fill=boxgreen!20, minimum width=3cm, minimum height=0.8cm] (prot) at (3,-2.5) {Protection};
    \draw[<->, thick] (inn) -- (prot);
\end{tikzpicture}
\end{center}
...
\footnotesize
\begin{description}
    \item[\textbf{Warren \& Brandeis}] (``The Right to Privacy,'' 1890) coined the phrase ``the right to be let alone''---the first systematic legal argument for privacy as an individual right, prompted by invasive newspaper photography.
    \item[\textbf{Helen Nissenbaum}]'s \textbf{contextual integrity} framework: privacy is not about secrecy but about appropriate information flows. A violation occurs when information moves to a context where it doesn't belong (e.g., medical data shared with employers).
    \item[\textbf{The chilling effect}]: surveillance changes behavior even when no one is being watched. The mere possibility of observation inhibits speech, association, and political dissent.
    \item[\textbf{The third-party doctrine}] (US law): information voluntarily shared with a third party (bank, phone company) loses Fourth Amendment protection. Digital life makes this doctrine cover almost everything.
    \item[\textbf{GDPR}] (EU General Data Protection Regulation, 2018): the most comprehensive privacy law to date. Key principles: data minimization, purpose limitation, right of access, right to erasure (``right to be forgotten'').
    \item[\textbf{Surveillance capitalism}] (Shoshana Zuboff): the business model of harvesting behavioral data to predict and modify human behavior for profit---going beyond mere data collection to behavioral modification.
\end{description}
