...
\mode<article>{
This lecture examines the philosophical foundations and practical challenges of intellectual property law in contexts of digital reproduction and distribution. Intellectual property differs fundamentally from physical property: ideas are non-rivalrous (my use need not prevent yours) and naturally non-excludable without legal intervention. We explore three competing justifications for IP rights---Locke's labor theory, utilitarian incentive arguments, and Hegelian personality theory---before considering how digital technology's capacity for perfect, costless copying has disrupted traditional IP business models and intensified debates about the appropriate balance between creator rights and public access to knowledge.
}

% ===== SLIDE 1: Title =====
\lecturetitleslide{images/fractal_04.png}
...
\begin{frame}{Before We Begin: When Did Mickey Mouse Stop Being Private Property?}
Two questions. Write down a number for each --- guesses only.

\begin{enumerate}
    \item Mickey Mouse first appeared in \emph{Steamboat Willie} in \textbf{1928}. In what year did that film enter the public domain, so that anyone could copy it freely?
    \item You write a song \textbf{today}. How long does your copyright last?
\end{enumerate}

\begin{discussionbox}[Commit before we go on]
Say your two numbers out loud. Whoever guessed the \textbf{longest} term for question 2 --- you are probably still too low.
\end{discussionbox}
\end{frame}

\begin{frame}{The Answer: 2024, and Your Whole Life Plus Seventy Years}
\begin{casestudybox}[Ninety-six years for a seven-minute cartoon]
\small
1928: \emph{Steamboat Willie} $\rightarrow$ 1976: the Copyright Act extends terms $\rightarrow$ 1998: the Sonny Bono Act extends them again $\rightarrow$ \textbf{January 1, 2024}: the film finally enters the public domain. Your own song gets \textbf{life plus 70 years}.
\end{casestudybox}

\begin{alertblock}{The question this course asks about that}
Copyright is justified as an \textbf{incentive to create}. Notice what a \emph{retroactive} extension can and cannot do: it cannot cause 1928 to be more creative. So what is it for, and who asked for it?
\end{alertblock}

We will come back to this --- along with the catch, which is that Disney still holds trademarks on Mickey and copyrights on every later version of him.
\end{frame}

% ===== SLIDE 2: Learning Outcomes and Central Questions =====
\begin{frame}[shrink=8]{Central Questions}
\begin{itemize}
    ...
    \item What ethical frameworks help us evaluate disputes---from Napster to AI-generated content?
    \item Do IP mechanisms primarily promote innovation, or primarily enable control by powerful interests?
    \item How do we balance creators' rights with the public's access to knowledge?
\end{itemize}

\vspace{0.2cm}
\begin{quotebox}[Thomas Jefferson, Letter to Isaac McPherson \parencite{jefferson_mcpherson}]
``He who receives an idea from me, receives instruction himself without lessening mine; as he who lights his taper at mine, receives light without darkening me.''
\end{quotebox}
\end{frame}

\section{Introduction: Intellectual Property in the Digital Age}

% ===== SLIDE 3: Connections to Previous Lectures =====
\begin{frame}{Connections to Previous Lectures}
\textbf{Lecture 1 (History)}: Each IT revolution promised liberation but enabled control. IP law is the legal mechanism attempting to thread this needle---protecting creators while managing public interest.

\vspace{0.2cm}
\textbf{Lecture 2 (Virtues)}: IP systems shape what kind of innovators and citizens we become. Does Copyright/Patent law encourage intellectual virtue (curiosity, intellectual humility) or vice (secrecy, rent-seeking)?

\vspace{0.2cm}
\textbf{Lecture 3 (Free Speech)}: IP and free speech tensions. Patents can restrict speech about inventions; copyright restricts quotation and remix; DMCA criminalizes circumvention speech. Balance?

\vspace{0.2cm}
\textbf{Looking ahead}: Lectures 5-8 will apply IP ethics to crypto, privacy, AI, and labor contexts.
\end{frame}
...
\begin{center}
\begin{tikzpicture}
    \node[rectangle, draw, fill=boxblue!20, rounded corners, minimum width=4cm, minimum height=1cm] (physical) at (0,0) {\textbf{Physical Property}};
    \node[rectangle, draw, fill=boxgreen!20, rounded corners, minimum width=4cm, minimum height=1cm] (ip) at (7,0) {\textbf{Intellectual Property}};
...
% ===== SLIDE 6: Copyright in Depth =====
\subsection{Deep Dives: Copyright, Patents, and Trade Secrets}
...
\end{frame}
% ===== IMAGE SLIDE: Thomas Edison =====
\begin{frame}[shrink=8]{Thomas Edison: The Patent Machine}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{block}{Edison (1847--1931): Master of the Patent}
\begin{itemize}
  \item Held \textbf{1,093 US patents}---a record that stood for decades
  \item Built the first \textbf{industrial research lab} at Menlo Park
  \item Inventions: phonograph, incandescent bulb, motion picture camera
  \item Fought the \textbf{``War of Currents''} against Tesla/Westinghouse
\end{itemize}
\end{block}
\begin{exampleblock}{The IP Question}
\small Did Edison's patents protect innovation---or lock down technology others could have improved?
\end{exampleblock}
\end{column}
\begin{column}{0.47\textwidth}
\begin{figure}
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{images/edison.jpg}
\caption{\scriptsize Thomas A.\ Edison, c.\ 1877--1880. Public domain.}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\mode<article>{
Intellectual property can be justified through three competing philosophical frameworks: the labor-desert theory (Locke) argues creators deserve to control their work; utilitarian theory argues IP incentivizes socially beneficial creation; personality theory (Hegel) argues ownership expresses the creator's personality. These justifications conflict in practice and lead to different policy conclusions. Understanding these frameworks helps evaluate whether current IP systems achieve their purported goals or instead serve incumbent interests.
}

% ===== IMAGE SLIDE: Plato on Ideas =====
\begin{frame}{A Philosophical Puzzle: Can You Own an Idea?}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{block}{Jefferson's Paradox}
\begin{itemize}
  \item Take my \textbf{car} and I lose it; copy my \textbf{idea} and I still have it
  \item Ideas are \textbf{non-rivalrous}---sharing doesn't diminish them
  \item IP law creates \textbf{artificial scarcity} where none exists
\end{itemize}
\end{block}
\begin{quotebox}[Thomas Jefferson, 1813]
\small ``He who receives an idea from me, receives instruction himself without lessening mine; as he who lights his taper at mine, receives light without darkening me.''
\end{quotebox}
\end{column}
\begin{column}{0.47\textwidth}
\begin{figure}
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{images/plato_bust.jpg}
\caption{\scriptsize Plato (428--348 BCE), Roman marble copy. Musei Capitolini, Rome. Public domain.}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}[shrink=8]{Justification 2: Utilitarian / Incentive Theory}
...
\begin{tikzpicture}[node distance=2cm]
    \node[rectangle, draw, fill=boxblue!20, rounded corners, minimum width=2.5cm, minimum height=0.7cm] (creator) {Creator};
    \node[rectangle, draw, fill=boxgold!30, rounded corners, minimum width=3cm, minimum height=0.7cm, right of=creator, xshift=2cm] (monopoly) {Limited Monopoly};
    \node[rectangle, draw, fill=teal!30, rounded corners, minimum width=2.5cm, minimum height=0.7cm, right of=monopoly, xshift=2cm] (public) {Public Domain};
...
\vspace{0.1cm}
\begin{quotebox}[James Boyle, \textit{The Public Domain} \parencite{boyle_public_domain}]
...
\section{Digital Disruption and Adaptation}

\mode<article>{
Digital technology fundamentally challenges intellectual property law's assumptions. Perfect, costless copying eliminates the natural scarcity that historically protected information goods. Global, instantaneous distribution makes enforcement difficult at individual level. The Napster case exemplifies the music industry's struggle to adapt, ultimately leading to new business models based on streaming rather than ownership. The DMCA's anti-circumvention provisions attempted to restore artificial scarcity through "digital locks" but created tensions with fair use, security research, and right to repair. The shift from ownership to access models represents a broader transformation in how IP functions in practice.
}

\subsection{How Technology Disrupted IP}
...
\section{Critiques and Abuses of IP}

\mode<article>{
Critical IP scholars like Peter Drahos, Cory Doctorow, and Tim Wu argue that contemporary IP law often serves concentrated corporate power rather than its stated goals of promoting innovation and creativity. Drahos's ``information feudalism'' describes how IP has been globalized through trade agreements to benefit wealthy countries and large corporations at developing nations' expense \parencite{drahos_information_feudalism}. Doctorow's ``chokepoint capitalism'' shows how platforms use IP to maintain bottleneck control over markets \parencite{doctorow_chokepoint}. Case studies including Disney's copyright extensions, pharmaceutical patent abuses, and patent trolls demonstrate how IP can become a tool for rent-seeking rather than innovation.
}

% ===== SLIDE 20: Peter Drahos =====
\begin{frame}{Peter Drahos: Information Feudalism}
\textbf{Key work}: \textit{Information Feudalism} \parencite{drahos_information_feudalism}
...
\textbf{Key work}: \textit{Chokepoint Capitalism} \parencite{doctorow_chokepoint}
...
\textbf{Key works}: \textit{The Master Switch} \parencite{wu_master_switch}, \textit{The Attention Merchants} \parencite{wu_attention_merchants}
...
% ===== SECTION III: BIG TECH AND IP =====

\section{Market Power and Control}

\mode<article>{
Large technology companies deploy intellectual property strategically as competitive weapons and defensive shields. Patents accumulate in vast portfolios enabling cross-licensing among giants while creating barriers for smaller competitors. Terms of service agreements function as IP-like control mechanisms without traditional IP law's limitations. The Oracle v. Google case over API copyrightability illustrates how fundamental questions about interoperability and competition depend on IP boundaries. The right to repair movement challenges how manufacturers use IP to restrict product modifications and repairs, raising questions about the meaning of ownership in digital contexts.
}

\subsection{IP as a Competitive and Defensive Weapon}
...
\section{Beyond Traditional IP: Open Source and Creative Commons}

\mode<article>{
The open source movement demonstrates that significant innovation and value creation can occur outside traditional IP incentive structures. Open source software licenses use copyright law itself to ensure works remain open rather than enclosed. Creative Commons provides similar frameworks for non-software content. These alternative models challenge utilitarian IP justifications by showing that other motivations---including reputation, community participation, and ideological commitment---can drive creation. The success of projects like Linux, Wikipedia, and countless open source libraries suggests that the optimal IP system may look quite different from current arrangements, particularly in contexts where collaborative and cumulative innovation predominate.
}
...
\section{AI and Intellectual Property}

\mode<article>{
Artificial intelligence creates novel IP challenges that existing legal frameworks struggle to address. The training of large language models on copyrighted works raises questions about whether such use constitutes infringement or transformative fair use. Multiple lawsuits by publishers, authors, and artists against AI companies are currently working through courts. Additionally, the authorship status of AI-generated content remains contested, with U.S. authorities currently requiring human authorship for copyright protection. These issues will significantly shape both AI development and IP law's evolution, potentially determining whether AI capabilities remain concentrated among well-resourced actors or become more democratically accessible.
}
...
\textbf{Key cases}: \textit{NYT v.\ OpenAI/Microsoft}, \textit{Getty v.\ Stability AI}, \textit{Kadrey v.\ Meta}, \textit{Bartz v.\ Anthropic}, \textit{Thomson Reuters v.\ Ross}.
\end{casestudybox}

\vspace{0.15cm}
\textbf{The claims}: Training without a license; models reproducing protected content; and---separately---\emph{how the training data was obtained}.

\vspace{0.15cm}
\textbf{Where things stand} (September 2026):
\begin{itemize}
    \item A dozen-plus suits against OpenAI/Microsoft form an \textbf{MDL} in S.D.N.Y.; summary-judgment motions filed Sept.\ 2026, with the \textbf{Justice Department} backing fair use
    \item Anthropic's \textbf{\$1.5 billion} settlement---largest in US copyright history---approved July 2026
    \item \textbf{No appeals court has yet ruled}; the first, \textit{Thomson Reuters v.\ Ross} (3d Cir., argued June 2026), involves a \emph{non-generative} tool
\end{itemize}

\vspace{0.1em}
{\scriptsize Almost nothing here is binding precedent yet. Expect the ground to shift during this semester.}
\end{frame}
% ===== YOU BE THE JUDGE =====
\begin{frame}[shrink=8]{You Be the Judge: The Authors v.\ the Model}
\begin{casestudybox}[The facts, stipulated]
\small
An AI company trained a language model on books. It bought and scanned some of them. It also downloaded and kept a permanent library of \textbf{millions of pirated books}. Authors sued.
\end{casestudybox}

\textbf{Three benches.} Rule on each act \emph{separately} --- fair use or not --- and say why:
\begin{enumerate}
    \item \textbf{Training} the model on the text
    \item \textbf{Scanning} books the company lawfully bought
    \item \textbf{Keeping} the pirated library
\end{enumerate}

\begin{discussionbox}[Before you see the real holding]
Did your three answers come out the same? If they did not, name the \textbf{principle} that separates them --- that principle is the whole case.
\end{discussionbox}
\end{frame}


% ===== SLIDE 34b: What Courts Have Actually Held =====
\begin{frame}{What the Courts Have Actually Held}
It is often said that ``courts ruled AI training is fair use.'' That is too coarse. \textit{Bartz v.\ Anthropic} (N.D.\ Cal.\ 2025) split the conduct apart:
...
\toprule
\textbf{Conduct} & \textbf{Holding} \\
\midrule
Training a model on the books & \textbf{Fair use} (``exceedingly transformative'') \\
Scanning \emph{lawfully purchased} print books & \textbf{Fair use} (format shift) \\
Keeping a permanent library of \textbf{7M+ pirated books} & \textbf{Not fair use} \\
\bottomrule
\end{tabular}
\end{table}

\vspace{0.2em}
Anthropic then paid \$1.5 billion---roughly \$3,000 per work across some 482,000 titles---over precisely that last row.

\begin{conceptbox}[The distinction worth remembering]
\scriptsize
The fight is turning less on \emph{what models learn} than on \textbf{how the corpus was acquired}. A separate theory, \textbf{market dilution}, asks whether a flood of AI output can harm the market for human work even when no passage is copied.
\end{conceptbox}
\end{frame}
...
\vspace{0.2cm}
\textbf{Current US position}:
\begin{itemize}
    \item Copyright requires \textbf{human authorship}---now settled, after the Supreme Court declined to review \textit{Thaler v.\ Perlmutter} in March 2026
    \item Purely AI-generated works are \textbf{not copyrightable}
    \item Human-AI collaboration: human contributions \textbf{may be} copyrightable, case by case
    \item \textbf{Prompts alone---however detailed---do not make you an author}
\end{itemize}

\vspace{0.15cm}
\textbf{The complications}: How much human input is \textbf{enough}? What about curation and selection? Different countries may reach different conclusions.
...
% ===== SLIDE 40: Conclusion - Key Terms and Ideas =====
\section{Conclusion: Intellectual Property in the Digital Age}
\begin{frame}[allowframebreaks]{Key Concepts}
\footnotesize
\begin{description}
    \item[\textbf{Locke's labor theory}] holds that creators deserve property rights over work that mixes their mental labor with the world---the most intuitive foundation for copyright and patent.
    \item[\textbf{Utilitarian / incentive theory}] justifies IP not by desert but by social benefit: exclusive rights incentivize creation that benefits everyone.
    \item[\textbf{Hegel's personality theory}] holds that creative works express the author's identity, justifying moral rights even after commercial transfer.
    \item[\textbf{Non-rivalrous goods}] are resources whose use by one party does not prevent use by another---ideas are naturally non-rivalrous, making IP's ``artificial scarcity'' a deliberate policy choice.
    \item[\textbf{Fair use}] permits unauthorized use for criticism, commentary, education, and parody; the key test courts apply is \textit{transformativeness}.
    \item[\textbf{DMCA (1998)}] criminalized circumvention of digital locks, prioritizing enforcement over fair use and blocking repair, security research, and accessibility.
    \item[\textbf{Copyleft}] (GPL) uses copyright's own mechanics to keep software open: any derivative must also remain open---IP turned into a tool for the commons.
    \item[\textbf{Enshittification}] (Doctorow \& Giblin) describes how IP and platform lock-in allow companies to progressively extract value from users and creators once competition is foreclosed.
\end{description}

\begin{discussionbox}[Discussion]
If you could redesign IP law from scratch, what would you prioritize: creator incentives, public access, or a healthy innovation ecosystem?
\end{discussionbox}
\end{frame}
