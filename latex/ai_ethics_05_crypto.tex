...
\mode<article>{
This lecture examines the political philosophy underlying cryptocurrency technologies. We begin by analyzing money as a social technology and exploring how different monetary systems embody different power relationships. Bitcoin \parencite{nakamoto_bitcoin} and subsequent cryptocurrencies represent an attempt to create money independent of state control through cryptographic proof and decentralized consensus, drawing on ideas from Friedrich Hayek's \textit{Denationalization of Money} \parencite{hayek_denationalization} and the cypherpunk movement \parencite{hughes_cypherpunk}. We evaluate arguments both for and against cryptocurrency from perspectives including financial inclusion, individual liberty, environmental impact, and the appropriate role of state power in monetary systems. The debate over cryptocurrency regulation crystallizes fundamental questions about the relationship between technology, markets, and political authority.
}

% Title slide
\lecturetitleslide{images/fractal_05.png}


% ====================
% COLD OPEN
% ====================
% Deliberately not named here -- the reveal is the Yap stone money slide later
% in Part I, and the thread pays off better if they have been sitting with it.

\begin{frame}{Before We Begin: Is This Money?}
A community uses the following currency:

\begin{itemize}
    \item The coins are \textbf{stone discs}, some four meters across and weighing four tonnes
    \item They are far \textbf{too heavy to move}, so nobody moves them. Ownership changes by \textbf{announcement}, and everyone remembers who owns what
    \item One of the coins fell off a boat and is \textbf{at the bottom of the ocean}. Its owner still owns it. Nobody disputes this. It still buys things
\end{itemize}

\begin{discussionbox}[Commit before we go on]
Is that money --- or is it a group of people agreeing to \emph{pretend} it is money? If you say it is not real money, be ready to say what the dollar in your pocket has that it lacks.
\end{discussionbox}
\end{frame}
...
\textbf{Lecture 1 (History)}: Every IT revolution disrupts existing economic intermediaries before new ones emerge. Is crypto's promise of disintermediation likely to follow the same reintermediation pattern?

\vspace{0.2cm}
\textbf{Lecture 3 (Free Speech)}: The cypherpunk movement grounded crypto in free speech and privacy values---encryption and financial anonymity as essential to liberty. Are these the same right?

\vspace{0.2cm}
\textbf{Lectures 4 \& 6 (IP, Privacy)}: Crypto transactions are pseudonymous, not anonymous. Who should have access to financial records---and under what conditions is surveillance of money flows justified?
\end{frame}
...
\mode<article>{
Money is a social technology whose form and control reflect underlying political arrangements. Throughout history, monetary systems have evolved from commodity money through representative currency backed by gold reserves to pure fiat currency based on government decree. Cryptocurrency represents a fourth paradigm: money based on cryptographic proof and algorithmic consensus rather than governmental or institutional authority. Understanding how money functions---as medium of exchange, store of value, and unit of account---is essential for evaluating cryptocurrency's promises and limitations. The history of banking crises helps explain why decentralized alternatives to traditional financial institutions appeal to many advocates.
}
...
% ===== IMAGE SLIDE: Lydian Coin =====
\begin{frame}{Ancient Money: The Lydian Electrum Coin}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{block}{The World's First Coins}
\begin{itemize}
  \item Lydia (modern Turkey) minted the first \textbf{standardized coins}, c.\ 600 BCE
  \item Made from \textbf{electrum}, a natural gold-silver alloy
  \item State-stamped to \textbf{certify weight and purity}---early trusted authority
  \item Enabled commerce between \textbf{strangers}
\end{itemize}
\end{block}
\begin{exampleblock}{The Insight}
\small Coins solved the \textbf{double coincidence of wants}: no need to find someone who wanted exactly what you had.
\end{exampleblock}
\end{column}
\begin{column}{0.47\textwidth}
\begin{figure}
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{images/lydian_coin.jpg}
\caption{\scriptsize Lydian electrum coin (\textit{trite}), c.\ 620--600 BCE. Public domain.}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\begin{tikzpicture}[scale=0.85]
    ...
    % Triangle
    \draw[thick, boxblue, fill=boxblue!5] (A) -- (B) -- (C) -- cycle;
    
    % Labels at vertices (in boxes)
    \node[above, font=\small\bfseries, text=boxblue] at (A) {Medium of Exchange};
    \node[below, font=\small\bfseries, text=boxblue, yshift=-3pt] at (B) {Store of Value};
    \node[below, font=\small\bfseries, text=boxblue, yshift=-3pt] at (C) {Unit of Account};
...
\begin{tikzpicture}[scale=0.75, transform shape,
    block/.style={rectangle, draw=boxblue, thick, fill=boxblue!10, 
    ...
    % Chain arrows
    \draw[->, thick, boxgold] (b1) -- (b2);
    \draw[->, thick, boxgold] (b2) -- (b3);
    \draw[->, thick, boxgold] (b3) -- (b4);
\end{tikzpicture}
\end{center}
...
\begin{frame}[shrink=8]{Bitcoin---The Original Cryptocurrency}
Created \textbf{January 3, 2009} by ``Satoshi Nakamoto'' \parencite{nakamoto_bitcoin} (pseudonymous, identity unknown).
...
\begin{itemize}
    \item Maximum supply: 21 million BTC ever; ``halving'' every $\sim$4 years slows new supply
    \item Emerged during the 2008 financial crisis---not coincidental
    \item Market cap: peaked above \$2 trillion (Oct.\ 2025), fell to about \$1.2 trillion (June 2026), back near \$1.7 trillion by September
\end{itemize}

{\scriptsize Treat that last figure as a snapshot: the asset shed about a third of its value in six months. The \emph{volatility} is the ethically interesting property---a currency that swings 30\% in half a year is hard to use as a unit of account.}

\begin{alertblock}{The Mystery}
\scriptsize
Satoshi disappeared in 2011. Estimated holdings: $\sim$1.1 million BTC---roughly \textbf{5\% of all Bitcoin that will ever exist}, unmoved since 2010. (Inferred from early-block patterns, not proof of ownership.)
\end{alertblock}
\end{frame}

% ===== IMAGE SLIDE: Yap Stone Money =====
\begin{frame}[shrink=8]{Yap Stone Money: Trust Without Technology}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{block}{Rai Stones of Yap (Micronesia)}
\begin{itemize}
  \item Giant limestone discs, up to 4 meters across and 4 tonnes
  \item \textbf{Too heavy to move}: ownership transferred verbally, kept in community memory
  \item One stone \textbf{sank at sea}---its owner still ``owned'' it, and everyone agreed
  \item Value rests entirely on \textbf{social consensus}, not possession
\end{itemize}
\end{block}
\begin{alertblock}{The Parallel}
\small Bitcoin's blockchain is a \textbf{digital ledger of consensus}---like the Yapese oral ledger, but in code.
\end{alertblock}
\end{column}
\begin{column}{0.47\textwidth}
\begin{figure}
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{images/yap_stone_money.jpg}
\caption{\scriptsize Rai (stone money), Yap, Federated States of Micronesia. Public domain.}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
Privacy advocates, cryptographers, and libertarians who believed: ``Privacy is necessary for an open society in the electronic age'' \parencite{hughes_cypherpunk}. Key figures: Timothy May, Eric Hughes, Nick Szabo, Hal Finney.
\end{casestudybox}
...
\textbf{Key thinkers}: Friedrich Hayek (\emph{Denationalization of Money}, 1976) \parencite{hayek_denationalization}, Murray Rothbard.
...
\begin{frame}[shrink=8]{Argument 1: Sound Money and Inflation Protection}
...
\begin{tikzpicture}[scale=0.85]
    ...
    \draw[thick, boxgold, domain=0:9.5, samples=50] 
        plot (\x, {4.76*(1-exp(-0.5*\x))});
    
    % Asymptote
    \draw[dashed, boxred] (0,4.76) -- (10,4.76);
    \node[right, font=\tiny, boxred] at (10,4.76) {21M cap};
...
\begin{tikzpicture}[scale=0.6]
    \node[circle, draw=boxred, fill=boxred!20, thick, minimum size=0.8cm] (center) at (0,0) {};
    \foreach \angle in {0,45,90,135,180,225,270,315} {
        \node[circle, draw=boxblue, fill=boxblue!20, minimum size=0.4cm] (n\angle) at (\angle:1.5) {};
...
\begin{tikzpicture}[scale=0.6]
    \foreach \i in {1,...,8} {
        ...
        \node[circle, draw=boxgreen, fill=boxgreen!20, minimum size=0.4cm] (n\i) at (\angle:1.5) {};
    }
    \foreach \i in {1,...,8} {
        \foreach \j in {1,...,8} {
            \ifnum\i<\j
...
\begin{itemize}
    ...
    \item Largest single heist: $\sim$\$1.4--1.5 billion from Bybit (February 2025)
    \item Funds weapons and missile programs
\end{itemize}
\end{frame}

% Slide 23: Critique - Who Actually Benefits?
\begin{frame}[shrink=8]{Critique: Who Actually Benefits from Crypto?}
...
\begin{frame}[shrink=8]{Critique: Is Cryptocurrency Actually Decentralized?}
...
\begin{itemize}
    ...
    \item \textbf{Wealth concentration}: $\sim$2\% of addresses hold $\sim$95\% of Bitcoin (caveat: addresses $\neq$ people---exchange addresses pool millions of users' funds)
...
\begin{tikzpicture}[scale=0.65]
    ...
    \fill[boxred!70] (0,0) -- (0:1.5) arc (0:7.2:1.5) -- cycle;
    \fill[boxblue!30] (0,0) -- (7.2:1.5) arc (7.2:360:1.5) -- cycle;
...
\begin{itemize}
    ...
    \item ``Per-transaction'' figures ($\sim$1,400 kWh) are a \textbf{contested metric}: mining energy doesn't scale with transaction count, so dividing one by the other can mislead
...
% ===== YOU BE THE JUDGE =====
\begin{frame}[shrink=8]{You Be the Judge: Ulbricht's Sentence}
\begin{casestudybox}[The record]
\small
Ross Ulbricht built and ran Silk Road. Convicted February 2015 on charges including narcotics trafficking and money laundering. Sentenced to \textbf{life without parole}, with no prior convictions. Pardoned by President Trump in \textbf{January 2025}.
\end{casestudybox}

\textbf{Three benches}: prosecution, defense, and the judge who has to sentence him.
\begin{itemize}
    \item \textbf{Prosecution}: he built the infrastructure knowing exactly what it was for
    \item \textbf{Defense}: he sold nothing; he wrote software, and adults chose what to do with it
    \item \textbf{The bench}: you may impose any sentence from probation to life
\end{itemize}

\begin{discussionbox}[The question behind the sentence]
Was life without parole a punishment for \textbf{what he did}, or for \textbf{what Silk Road represented}? Does the 2025 pardon change your answer --- and should a pardon be able to?
\end{discussionbox}
\end{frame}
...
2025 & \$2.02 billion & Bybit ($\sim$\$1.4--1.5B) \\
\midrule
\textbf{Total} & \textbf{\$6.75 billion} & Funds weapons programs \\
\bottomrule
\end{tabular}
\caption{\scriptsize Source: Chainalysis year-end data, Dec.\ 2025}
\end{table}
\vspace{-0.5em}
...
\begin{frame}[shrink=8]{Argument 5: Regulatory Arbitrage and Tax Evasion}
...
\begin{itemize}
    ...
    \item Ordered to forfeit \$11 billion; \textbf{upheld on appeal} (June 2026)
\end{itemize}
...
\begin{frame}[shrink=8]{Critique: Traditional Finance Also Has Problems}
...
\begin{frame}[shrink=8]{Critique: The International Coordination Problem}
...
% Slide 39: CBDCs
\begin{frame}[shrink=8]{Central Bank Digital Currencies (CBDCs)---A Middle Path?}
...
\midrule
Bahamas & Launched (2020) & Sand Dollar---first CBDC; under 1\% of currency in circulation \\
Jamaica & Launched (2022) & JAM-DEX; stalled on merchant terminals \\
Nigeria & Launched (2021) & eNaira: $\sim$13M wallets, \textbf{98.5\% never used} \\
China & Pilot & e-CNY: 230M personal wallets \\
EU & In negotiation & Digital Euro: pilot 2027, issuance 2029 at the earliest \\
\textbf{USA} & \textbf{Prohibited} & By executive order (2025); statutory ban still pending \\
\bottomrule
\end{tabular}
\caption{\scriptsize Atlantic Council CBDC Tracker ($\sim$146 jurisdictions exploring, mid-2026)}
\end{table}

{\scriptsize \textbf{Benefits}: financial inclusion, payment efficiency, monetary policy tools. \textbf{Concerns}: \textbf{total financial surveillance}, programmable restrictions.

\textbf{Note the pattern}: every country that launched a retail CBDC struggles with adoption. Nigeria repositioned the eNaira as back-end infrastructure in 2026, while a \emph{privately} issued naira stablecoin gained traction voluntarily. Can adoption be mandated?}
\end{frame}
...
\footnotesize
\begin{description}
    \item[\textbf{Satoshi Nakamoto}] (pseudonym) published the Bitcoin whitepaper in 2008, proposing a peer-to-peer electronic cash system that eliminates the need for trusted third parties through cryptographic proof.
    \item[\textbf{Friedrich Hayek}] (\textit{Denationalization of Money}, 1976) argued that government monopolies on currency cause inflation and instability; competing private currencies would serve citizens better---a key intellectual ancestor of crypto ideology.
    \item[\textbf{The cypherpunk movement}] (1980s--90s) argued that privacy is necessary for an open society and that cryptography is the tool to achieve it. Bitcoin emerged from this tradition.
    \item[\textbf{Fiat currency}] is money backed by government authority alone (not gold or commodities). The 1971 Nixon Shock ended dollar-gold convertibility, inaugurating the current purely fiat era.
    \item[\textbf{Blockchain}] is the distributed ledger technology underlying most cryptocurrencies: a chain of cryptographically linked blocks, replicated across nodes, making records tamper-evident without a central authority.
    \item[\textbf{Proof of Work}] requires miners to solve computationally expensive puzzles to add blocks---creating the energy consumption critique. \textbf{Proof of Stake} (Ethereum's current system) uses economic deposits instead.
    \item[\textbf{CBDCs}] (Central Bank Digital Currencies) adopt blockchain technology while keeping state control---offering efficiency but raising concerns about total financial surveillance.
\end{description}
