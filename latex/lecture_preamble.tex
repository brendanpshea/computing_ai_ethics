% ==============================================================================
% Common Preamble for IT Ethics Lectures
% ==============================================================================
% This file contains shared configuration for all lecture files.
% Include this after \documentclass using: \input{lecture_preamble.tex}
%
% Individual lecture files should:
% 1. Declare \documentclass (beamer or beamerarticle)
% 2. \input{lecture_preamble.tex}
% 3. Define lecture-specific colors/customizations
% 4. Set title information
% ==============================================================================

% ------------------------------------------------------------------------------
% Theme and Basic Setup
% ------------------------------------------------------------------------------
\usetheme[progressbar=foot, block=fill, sectionpage=progressbar]{metropolis}

% ------------------------------------------------------------------------------
% Core Packages
% ------------------------------------------------------------------------------
\usepackage{tikz}
\usetikzlibrary{shapes,arrows,positioning,calc,decorations.pathreplacing,fit,backgrounds,chains}
\usepackage{booktabs}
\usepackage{array}
\usepackage{graphicx}
\usepackage{xcolor}
\usepackage[sfdefault]{plex-sans}   % IBM Plex Sans — compact, modern, tech-themed
\usepackage[T1]{fontenc}
\usepackage{fontawesome5}          % real vector icons

% Conditional text boxes (more flexible than basic beamer blocks)
\usepackage{tcolorbox}
\tcbuselibrary{skins,breakable}

% ------------------------------------------------------------------------------
% Bibliography (loaded in all modes for citations)
% ------------------------------------------------------------------------------
\usepackage[style=authoryear,backend=biber,natbib=true,maxcitenames=2]{biblatex}
\addbibresource{refs.bib}

% ------------------------------------------------------------------------------
% Article Mode Adjustments (NOT MAINTAINED)
% ------------------------------------------------------------------------------
% The article/handout build is not used or maintained (see docs/BUILD.md).
% \mode<article> blocks in the lectures are ignored by the normal beamer build.
% When compiling as article (using beamerarticle class), apply these settings
\mode<article>{
    % Use standard article sections instead of beamer frames
    \usepackage{fullpage}
    \usepackage{hyperref}
    \hypersetup{
        colorlinks=true,
        linkcolor=blue,
        citecolor=blue,
        urlcolor=blue
    }

    % Redefine frame environment for article mode
    % Frames become subsections in article mode
    \renewenvironment{frame}[1][]{
        \par\medskip
        \noindent\textbf{#1}\par\medskip
    }{}
}

% Presentation mode specific settings
\mode<presentation>{
    \setlength{\parskip}{0pt}

    % Show a TOC slide at the start of each section, current section highlighted
    \AtBeginSection[]{
        \begin{frame}[shrink=15]{Lecture Outline}
            \tableofcontents[currentsection, hideallsubsections]
        \end{frame}
    }

    % IBM Plex Sans (loaded above) runs tall relative to Computer Modern; pull line
    % spacing back to avoid the overflows Metropolis + a tall sans produce by default.
    \renewcommand{\baselinestretch}{0.88}

    % Tighter list spacing
    \setbeamertemplate{itemize/enumerate body begin}{\setlength{\itemsep}{2pt}\setlength{\parsep}{0pt}}
    \setbeamertemplate{itemize/enumerate subbody begin}{\setlength{\itemsep}{1pt}\setlength{\parsep}{0pt}}

    % Captions here are image credits and source lines, not numbered figure
    % references -- nothing in the decks says "see Figure 3". Drop the
    % "Figure N:" prefix so \caption prints just the credit text.
    %
    % Do NOT use \caption* to achieve this: beamer does not define the starred
    % form, so \caption*{Credit} silently renders as "Figure N: *" followed by
    % the credit as body text. Always use plain \caption here.
    \setbeamertemplate{caption}{\raggedright\insertcaption\par}
    \setbeamertemplate{caption label separator}{}
    \setbeamerfont{caption}{size=\scriptsize}
}

% ------------------------------------------------------------------------------
% [shrink=N]: compress vertically only, and only as much as needed
% ------------------------------------------------------------------------------
% Stock beamer does two things we do not want on these decks.
%
%   1. It scales uniformly (\pgftransformscale) and pre-widens \hsize by 1/factor,
%      so a frame that overruns only vertically also comes out in smaller type
%      with longer lines than every neighbouring slide. Our frames overflow
%      because there is one line too many, not because they are too wide.
%   2. It always applies the full declared percentage. The "only shrink as much
%      as necessary" test is commented out in beamerbaseframesize.sty, so
%      [shrink=15] shrinks by 15% even when 3% would have cleared the overflow.
%
% The redefinitions below scale the y axis (\pgftransformyscale, glyph widths and
% the text block width untouched) and compute the factor from the actual overrun,
% so a frame that fits is left alone entirely. The declared N becomes a budget
% rather than a command: if clearing the overflow needs more than N, beamer says
% so in a warning and that slide is a candidate for cutting.
%
% Anamorphic squeeze is capped at \beamer@maxsquash. Up to that point it reads as
% tighter leading and nothing else; past it, text starts to look rubbery and any
% photo on the slide is visibly flattened. A frame that needs more than the cap
% takes the cap vertically and the remainder as an ordinary uniform scale -- i.e.
% it degrades to stock beamer's behaviour rather than to a distorted slide.
%
% Written at top level rather than inside \mode<presentation>{...}: that form
% reads its body as an argument, so catcodes are already fixed and the
% \makeatletter never takes effect on the \...@... names below. The guard keeps
% article mode (where beamer defines no such macro) out of it instead.
\makeatletter
\@ifundefined{beamer@shrinkframebox}{}{%
    \def\beamer@maxsquash{0.8}%      most the y axis may be squeezed (20%)
    \def\beamer@maxsquashinv{1.25}%  must be 1/\beamer@maxsquash
    \define@key{beamerframe}{shrink}[0]{%
        \def\beamer@shrinkpercentage{#1}%
        \@tempdima=-#1pt%
        \@tempdima=0.01\@tempdima%
        \advance\@tempdima by1pt%
        \edef\beamer@shrinkfactor{\strip@pt\@tempdima}%
        \def\beamer@shrinkfactorinv{1}% no horizontal pre-widening
        \beamer@shrinktrue%
        \def\beamer@cramped{\itemsep0pt\topsep0.5pt\relax}}
    \def\beamer@shrinkframebox{%
        % Height AND depth: stock beamer looks at \ht alone, which it gets away
        % with because it always scales. Since we scale only when the frame
        % overruns, a [T] column or a graphic that hangs below the baseline has
        % to count, or the frame is left alone and still runs off the slide.
        \@tempdima=\ht\beamer@framebox%
        \advance\@tempdima by\dp\beamer@framebox%
        \ifdim\@tempdima>\beamer@frametextheight% only act if it really overflows
            \@tempcnta=\@tempdima%
            \divide\@tempcnta by 65536\relax%
            \@tempdimc=\beamer@frametextheight%
            \divide\@tempdimc by\@tempcnta\relax%
            \ifdim\@tempdimc<\beamer@shrinkfactor pt%
                \@tempdima=-100\@tempdimc%
                \advance\@tempdima by100pt%
                \ClassWarning{beamer}{Frame compressed vertically by \strip@pt\@tempdima\space
                    percent instead of \beamer@shrinkpercentage\space percent -- consider cutting this slide}%
            \fi%
            % Split the needed factor into a capped vertical squeeze and, only if
            % that is not enough, a uniform scale carrying the remainder.
            \@tempdimb=1pt%
            \ifdim\@tempdimc<\beamer@maxsquash pt%
                \@tempdimb=\beamer@maxsquashinv\@tempdimc%
                \@tempdimc=\beamer@maxsquash pt%
                \ClassWarning{beamer}{Past the vertical-squeeze cap; the rest of this
                    frame is being scaled down uniformly, so its type is smaller than
                    the surrounding slides -- cut content here}%
            \fi%
            \edef\beamer@yfactortemp{\strip@pt\@tempdimc}%
            \edef\beamer@factortemp{\strip@pt\@tempdimb}%
            \setbox\beamer@framebox=\vbox to\beamer@frametextheight{%
                \begin{pgfpicture}{0pt}{-\beamer@frametextheight}{0pt}{0pt}
                    {\pgftransformscale{\beamer@factortemp}%
                        \pgftransformyscale{\beamer@yfactortemp}%
                        \pgftext[left,top]{\box\beamer@framebox}}%
                \end{pgfpicture}%
            }%
        \fi%
    }%
}
\makeatother

% ------------------------------------------------------------------------------
% Standard Colors for Boxes
% ------------------------------------------------------------------------------
% Consistent color scheme across all lectures for semantic elements
\definecolor{boxblue}{HTML}{1A4A7A}       % Arguments, concepts
\definecolor{boxgreen}{HTML}{2E7D32}      % Pro arguments, supporting points
\definecolor{boxred}{HTML}{C0392B}        % Objections, warnings
\definecolor{boxgold}{HTML}{B7770D}       % Case studies, examples
\definecolor{boxgray}{HTML}{555F6D}       % Quotes, citations
\definecolor{boxpurple}{HTML}{6A3093}     % Secondary accent
% Metropolis structural colors
\definecolor{mPrimary}{HTML}{1E3A5F}      % Deep navy — frametitle / title bg
\definecolor{mAccent}{HTML}{D4880A}       % Warm amber — progress bar / separators

% ------------------------------------------------------------------------------
% Beamer Theme Color Setup
% ------------------------------------------------------------------------------
% Applied once here; individual lectures should NOT override these.
\mode<presentation>{
    % Metropolis palette primary drives frametitle + title frame background
    \setbeamercolor{palette primary}{bg=mPrimary, fg=white}
    \setbeamercolor{frametitle}{bg=mPrimary, fg=white}
    % Title page element colors — must be explicit so they stay white
    % when a \usebackgroundtemplate image replaces the default navy fill
    \setbeamercolor{title}{fg=white}
    \setbeamercolor{subtitle}{fg=mAccent}
    \setbeamercolor{author}{fg=white}
    \setbeamercolor{institute}{fg=white!85!black}
    \setbeamercolor{date}{fg=white!75!black}
    % Progress bar and title separator use accent color
    \setbeamercolor{progress bar}{fg=mAccent, bg=mAccent!25}
    \setbeamercolor{title separator}{fg=mAccent}
    \setbeamercolor{alerted text}{fg=boxred}
    % Standard beamer blocks
    \setbeamercolor{block title}{fg=white, bg=boxblue}
    \setbeamercolor{block body}{bg=boxblue!8}
    \setbeamercolor{block title alerted}{fg=white, bg=boxred}
    \setbeamercolor{block body alerted}{bg=boxred!8}
    \setbeamercolor{block title example}{fg=white, bg=boxgreen}
    \setbeamercolor{block body example}{bg=boxgreen!5}
}

% ------------------------------------------------------------------------------
% Standard Custom Boxes
% ------------------------------------------------------------------------------
% Uniform boxes used across all lectures - do not redefine in individual files

% Argument/Reasoning Box (logical arguments in standard form)
\newtcolorbox{argumentbox}[1][Argument]{
    colback=boxblue!5,
    colframe=boxblue,
    fonttitle=\bfseries,
    title={#1},
    boxrule=1pt,
    arc=2pt
}

% Quotation Box (for philosophical quotes, sources)
\newtcolorbox{quotebox}[1][Quote]{
    colback=gray!10,
    colframe=boxgray,
    fonttitle=\itshape,
    title={#1},
    boxrule=0.5pt,
    arc=2pt,
    left=10pt,
    right=10pt,
    leftrule=3pt
}

% Case Study/Example Box
\newtcolorbox{casestudybox}[1][Case Study]{
    colback=boxgold!10,
    colframe=boxgold,
    fonttitle=\bfseries,
    title={#1},
    boxrule=1pt,
    arc=2pt
}

% Objection/Counterargument Box
\newtcolorbox{objectionbox}[1][Objection]{
    colback=boxred!5,
    colframe=boxred,
    fonttitle=\bfseries,
    title={#1},
    boxrule=1pt,
    arc=2pt
}

% Pro/Supporting Argument Box
\newtcolorbox{probox}[1][Supporting Argument]{
    colback=boxgreen!5,
    colframe=boxgreen,
    fonttitle=\bfseries,
    title={#1},
    boxrule=1pt,
    arc=2pt
}

% Concept/Definition Box
\newtcolorbox{conceptbox}[1][Key Concept]{
    colback=boxblue!5,
    colframe=boxblue,
    fonttitle=\bfseries,
    title={#1},
    boxrule=1pt,
    arc=2pt
}

% Discussion Box (for group activities, questions)
% Compact padding: these appear at slide bottoms very often, so keep the
% vertical footprint close to a beamer block to avoid pushing content off-slide.
\newtcolorbox{discussionbox}[1][Discussion]{
    colback=boxgold!5,
    colframe=boxgold,
    fonttitle=\bfseries,
    title={#1},
    boxrule=1pt,
    arc=2pt,
    boxsep=1pt,
    top=2pt, bottom=2pt, left=5pt, right=5pt,
    toptitle=1pt, bottomtitle=1pt
}

% Figure Box (for diagrams with explanations)
\newtcolorbox{figurebox}[1][Figure]{
    colback=boxblue!3,
    colframe=boxblue!70,
    fonttitle=\bfseries,
    title={#1},
    boxrule=0.75pt,
    arc=2pt
}

% ------------------------------------------------------------------------------
% Utility Commands and Icons
% ------------------------------------------------------------------------------

% fontawesome5 icon shorthands (\faXxx commands come from the fontawesome5 package)
\newcommand{\iconQuote}{\faQuoteLeft}
\newcommand{\iconBalance}{\faBalanceScale}
\newcommand{\iconDiscussion}{\faComments}
\newcommand{\iconIdea}{\faLightbulb}
\newcommand{\iconPro}{\faThumbsUp}
\newcommand{\iconCon}{\faThumbsDown}

% Discussion question command (for interactive moments)
% Discussion prompts use the gold discussionbox (inviting), never the red
% alertblock (which is reserved for warnings and key tensions).
\newcommand{\discussionquestion}[1]{%
    \begin{discussionbox}[Discussion]
    #1
    \end{discussionbox}
}

% fontawesome5 defines \faQuoteLeft, \faBalanceScale, etc. natively --- no aliases needed.
\newcommand{\discussion}[1]{\discussionquestion{#1}}

% ------------------------------------------------------------------------------
% Standard Title Slide
% ------------------------------------------------------------------------------
% Uniform title slide for all lectures: a full-bleed background image with a
% dark tcolorbox overlay so the title text stays legible over any image.
% Usage (immediately after \begin{document}):
%   \lecturetitleslide{images/fractal_01.png}
\newcommand{\lecturetitleslide}[1]{%
{%
\usebackgroundtemplate{\includegraphics[width=\paperwidth,height=\paperheight]{#1}}%
\begin{frame}
\vfill
\begin{center}
\begin{tcolorbox}[
    enhanced,
    width=0.84\textwidth,
    colback=black!82,
    colframe=white!85,
    boxrule=0.6pt,
    arc=3pt,
    left=10pt,
    right=10pt,
    top=10pt,
    bottom=10pt
]
{\centering
{\usebeamerfont{title}\color{white}\inserttitle\par}
\vspace{0.35em}
{\usebeamerfont{subtitle}\color{mAccent}\insertsubtitle\par}
\vspace{0.8em}
{\usebeamerfont{author}\color{white}\insertauthor\par}
{\usebeamerfont{institute}\color{white!85}\insertinstitute\par}
{\usebeamerfont{date}\color{white!75}\insertdate\par}
}
\end{tcolorbox}
\end{center}
\vfill
\end{frame}
}%
}

% Simple divider for slide sections
\newcommand{\sectiondivider}{%
    \begin{center}
    \rule{0.8\textwidth}{0.4pt}
    \end{center}
}

% Argument environment (for standard form arguments)
\newenvironment{argument}{%
  \begin{argumentbox}[Argument in Standard Form]
  \begin{enumerate}
}{%
  \end{enumerate}
  \end{argumentbox}
}

% Quote slide environment (combines icon and quotebox)
\newenvironment{quoteslide}[2]{%
  \begin{quotebox}[#1]
  \small
  \textit{``#2''}
}{%
  \end{quotebox}
}

% ==============================================================================
% End of Common Preamble
% ==============================================================================
