...
\mode<article>{
This lecture examines how modern technology, particularly social media and artificial intelligence, affects human character and flourishing from the perspective of virtue ethics. Drawing on Aristotle's ethical framework \parencite{aristotle_nicomachean} and contemporary applications by \textcite{vallor_technology_virtues} and others, we explore whether digital technologies are systematically undermining our capacity to develop intellectual and moral virtues. The central question is not simply ``What should we do?'' but ``What kind of people are we becoming?''
}

% ===== SLIDE 1: Title =====
\lecturetitleslide{images/fractal_02.png}


% ====================
% COLD OPEN
% ====================
% The data is on their own phones, so nothing here can go stale.

\begin{frame}{Before We Begin: Take Out Your Phone}
\textbf{Guess first, then check.} Write down two numbers before you look:
\begin{enumerate}
    \item Your \textbf{daily average screen time} over the past week
    \item How many times you \textbf{picked the phone up} yesterday
\end{enumerate}

Now look them up --- iPhone: \emph{Settings $\rightarrow$ Screen Time}; Android: \emph{Settings $\rightarrow$ Digital Wellbeing}.

\begin{alertblock}{Why we are starting here}
Aristotle's central claim is that you become what you repeatedly do. If he is right, those two numbers are not a report about your phone. They are a report about \textbf{your character} --- and about the person the next four years are building.
\end{alertblock}

\begin{discussionbox}[Commit before we go on]
Was your guess high or low? \textbf{Which direction did the class miss in}, and what would Aristotle make of the fact that we miss in that direction?
\end{discussionbox}
\end{frame}
...
\begin{itemize}
    ...
    \item What makes technology a moral concern---not just a tool but a shaper of who we are becoming?
    ...
    \item Are virtue ethics arguments for technology regulation compatible with individual liberty?
\end{itemize}

\vspace{0.2cm}
\begin{quotebox}[Will Durant, summarizing Aristotle's \textit{Nicomachean Ethics} \parencite{aristotle_nicomachean}]
``We are what we repeatedly do. Excellence, then, is not an act, but a habit.''

\vspace{0.2em}
\scriptsize (Often misattributed to Aristotle himself---a nice lesson in checking sources!)
\end{quotebox}
\end{frame}

\section{Introduction: Virtue Ethics and Technology}

\begin{frame}{Connections to Previous Lectures}
\textbf{Lecture 1 (History)}: Every IT revolution changes not just what people do but how they think. Writing atrophied memory; mass media enabled propaganda. How might social media and AI reshape character at scale?

\vspace{0.2cm}
\textbf{Looking ahead}:
\begin{itemize}
    \item \textbf{Lecture 3 (Free Speech)}: If the self is shaped by online discourse, what follows for speech regulation?
    \item \textbf{Lectures 4--6 (IP, Crypto, Privacy)}: Platform design choices affect users' character---who is responsible?
    \item \textbf{Lectures 7--8 (AI, Work)}: AI systems that learn from human behavior may reinforce vices at scale.
\end{itemize}
\end{frame}
...
% ===== IMAGE SLIDE: Aristotle Bust =====
\begin{frame}{Aristotle in Marble}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{block}{The Philosopher (384--322 BCE)}
\begin{itemize}
  \item Studied at \textbf{Plato's Academy} for 20 years before founding his own school
  \item His \textbf{Lyceum} became one of the ancient world's great centers of learning
  \item Tutor to the young \textbf{Alexander the Great}
  \item Medieval scholars simply called him ``\textbf{the Philosopher}''---no other name needed
  \item His \textit{Nicomachean Ethics} remains foundational to moral philosophy
\end{itemize}
\end{block}
\end{column}
\begin{column}{0.47\textwidth}
\begin{figure}
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{images/aristotle_bust.jpg}
\caption{\scriptsize Aristotle, Roman marble copy of a Greek bronze by Lysippos, c.\ 330 BCE. Museo Nazionale Romano, Rome. Public domain.}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\begin{tikzpicture}[node distance=3.5cm, auto,
    block/.style={rectangle, draw, fill=boxblue!20, text width=3cm, text centered, rounded corners, minimum height=1cm}]
...
\begin{tikzpicture}
    ...
    % Vice (deficiency)
    \node[fill=boxred!30, draw=boxred, rounded corners, minimum width=2cm, minimum height=0.8cm] at (1.5,0) {\small Cowardice};
    
    % Virtue (mean)
    \node[fill=boxgreen!30, draw=boxgreen, rounded corners, minimum width=2cm, minimum height=0.8cm] at (5,0) {\small \textbf{Courage}};
    
    % Vice (excess)
    \node[fill=boxred!30, draw=boxred, rounded corners, minimum width=2cm, minimum height=0.8cm] at (8.5,0) {\small Recklessness};
...
\begin{tikzpicture}[node distance=1.8cm,
    process/.style={rectangle, draw, fill=boxblue!15, text width=2.8cm, text centered, rounded corners, minimum height=0.9cm, font=\small}]
...
\begin{center}
\begin{tikzpicture}[node distance=1.8cm,
    block/.style={rectangle, draw, fill=boxgreen!20, text width=1.6cm, text centered, rounded corners, minimum height=0.7cm, font=\small}]
...
% ===== IMAGE SLIDE: Raphael's School of Athens =====
\begin{frame}{Plato and Aristotle: The School of Athens}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{block}{Raphael's Masterpiece (1509--1511)}
\begin{itemize}
  \item Painted for Pope Julius II's private library in the Vatican
  \item The great thinkers of antiquity gathered in an idealized Athens
  \item \textbf{Center}: Plato (pointing up---Ideal Forms) and Aristotle (hand level---observation)
\end{itemize}
\end{block}
\begin{exampleblock}{The Contrast}
\small Plato's ethics flows from ideal Forms; Aristotle's from observing how virtuous people actually live.
\end{exampleblock}
\end{column}
\begin{column}{0.47\textwidth}
\begin{figure}
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{images/raphael_school_athens.jpg}
\caption{\scriptsize Raphael Sanzio, \textit{The School of Athens}, 1509--1511. Apostolic Palace, Vatican City. Public domain.}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\textbf{After Virtue} \parencite{macintyre_after_virtue}: Diagnosis of moral fragmentation
...
\vspace{0.3cm}
\begin{quotebox}[MacIntyre \parencite{macintyre_after_virtue}]
...
\section{Intellectual Virtues Under Threat}

\mode<article>{
Intellectual virtues---including curiosity, intellectual humility, love of truth, open-mindedness, and intellectual courage---are essential for human flourishing. These virtues are cultivated through practices like sustained attention, effortful memory work, and engagement with diverse viewpoints. However, contemporary digital technologies may systematically undermine these practices by rewarding distraction, outsourcing cognitive work, creating echo chambers, and fostering intellectual dependency through AI tools.
}
...
\vspace{0.3cm}
\begin{quotebox}[William James \parencite{james_principles}]
...
\textbf{Nicholas Carr}, \textit{The Shallows} \parencite{carr_shallows} and \textbf{Jonathan Haidt} \parencite{haidt_anxious_generation}:
...
\begin{itemize}
    ...
    \item Estimates of daily phone checks vary widely across studies ($\sim$60--150+)---treat any precise number with caution
\end{itemize}
...
\begin{tabular}{l c c}
\toprule
\textbf{Metric} & \textbf{2004} & \textbf{2020s} \\
\midrule
Avg.\ time on one screen before switching & $\sim$2.5 minutes & $\sim$47 seconds \\
\bottomrule
\end{tabular}
\caption{\small Gloria Mark's workplace-observation studies (\textit{Attention Span}, 2023). Note: this measures \textbf{screen-switching}, not ``attention span''---the headline version of this stat overstates the finding.}
\end{table}
...
\vspace{0.3cm}
\begin{quotebox}[Vallor, \textit{Technology and the Virtues} \parencite{vallor_technology_virtues}]
...
\begin{tikzpicture}[node distance=1.5cm,
    evidence/.style={rectangle, draw, fill=boxred!15, text width=3cm, text centered, rounded corners, minimum height=0.8cm, font=\small},
    result/.style={rectangle, draw, fill=boxred!30, text width=4cm, text centered, rounded corners, minimum height=1cm, font=\small\bfseries}]
...
\section{Moral Virtues Under Threat}

\mode<article>{
Moral virtues such as empathy, compassion, patience, temperance, civility, and honesty are traditionally cultivated through embodied, face-to-face social interactions that involve vulnerability and reciprocity. Digital mediation of human relationships may degrade these practices. Online environments characterized by anonymity, asynchronicity, and algorithmic amplification of outrage may actively habituate users toward moral vices including cruelty, impatience, and dishonesty. Evidence from empathy research, studies of online disinhibition, and analysis of social media business models suggests cause for concern.
}
...
\textbf{Sherry Turkle}, \textit{Reclaiming Conversation} \parencite{turkle_reclaiming}:
...
\vspace{0.3cm}
\begin{quotebox}[Turkle \parencite{turkle_reclaiming}]
...
\textbf{University of Michigan meta-study} \parencite{konrath_empathy_decline}:
...
\begin{tikzpicture}
    ...
    \draw[thick, boxred] (0.5,2.5) -- (3,2.3) -- (5,1.8) -- (7.5,1.5);
...
{\scriptsize Illustrative sketch of the trend reported by Konrath et al.---not plotted from the actual data.}
\end{center}
\end{frame}
...
\begin{center}
\begin{tikzpicture}[node distance=1.8cm,
    block/.style={rectangle, draw, fill=boxred!15, text width=2.2cm, text centered, rounded corners, minimum height=0.7cm, font=\footnotesize}]
...
\section{Eudaimonia Under Threat}

\mode<article>{
Beyond specific virtues, Aristotelian ethics focuses on eudaimonia---human flourishing across a complete life. Eudaimonia requires integrated exercise of virtue, meaningful relationships, purposeful activity, and genuine self-knowledge. Contemporary evidence suggests that social media and AI may fragment attention, commodify relationships, and distort self-understanding in ways that directly impede the conditions necessary for flourishing. Rising rates of depression and anxiety, particularly among adolescents, coinciding with smartphone adoption provide troubling empirical support for these concerns.
}
...
\textbf{Jonathan Haidt}, \textit{The Anxious Generation} \parencite{haidt_anxious_generation}:
...
\begin{tikzpicture}[scale=0.9]
    ...
    \draw[thick, boxblue] (0.5,0.6) -- (3,0.7) -- (4,0.8);
    \draw[thick, boxred] (4,0.8) -- (5,1.2) -- (6,1.6) -- (7.5,2.2);
...
{\scriptsize Illustrative sketch of the pattern Haidt describes---not plotted from the actual data.}
\end{center}
\end{frame}

% ===== SLIDE 31b: The Causation Debate =====
\begin{frame}{But Is It Causal? The Haidt Debate}
\begin{objectionbox}[The Skeptics' Critique (Candice Odgers; Orben \& Przybylski)]
\small
\begin{itemize}
    \item The evidence is largely \textbf{correlational}---and the correlations are small
    \item One large-dataset analysis: social media explained less variance in teen well-being than \textbf{eating potatoes} or \textbf{wearing glasses}
    \item \textbf{Reverse causation} is plausible: struggling teens may turn to social media
    \item Rival explanations for the post-2012 rise: economic anxiety, academic pressure, better screening and reporting
\end{itemize}
\end{objectionbox}

\vspace{0.15cm}
{\small \textbf{Haidt's reply}: deactivation \emph{experiments} show effects too, and the timing replicates across many countries at once.}

\vspace{0.1cm}
\begin{alertblock}{The Methodological Lesson}
\small A \textbf{live scientific debate}, not settled fact---hold the hypothesis seriously, but provisionally.
\end{alertblock}
\end{frame}
...
\vspace{0.3cm}
\begin{quotebox}[Turkle \parencite{turkle_alone_together}]
...
\begin{frame}[shrink=8]{Premise 2c: Distorted Self-Understanding}
\textbf{Algorithmic identity}: You are what the algorithm thinks you are

\begin{itemize}
    \item \textbf{Filter bubbles} as distorting mirrors
    \item The algorithm shows ``more of what you'll predictably attend to''---but is that \textit{you}?
    \item It \textbf{shapes your behavior} to maximize engagement---which may not align with your flourishing
\end{itemize}

\begin{center}
\begin{tikzpicture}[scale=0.8, transform shape, node distance=2.2cm,
    block/.style={rectangle, draw, fill=boxgray!15, text width=2.2cm, text centered, rounded corners, minimum height=0.7cm, font=\footnotesize}]
...
\mode<article>{
If the virtue ethics diagnosis is correct, what practical responses follow? Individual responses like digital detoxes and mindful technology use have value but may be insufficient given the structural forces at play. Virtue ethics can ground arguments for regulatory interventions aimed at creating environments more conducive to flourishing, such as age restrictions on social media, design mandates that remove features encouraging vice, algorithm transparency requirements, and comprehensive digital literacy education. The challenge is balancing protection of human flourishing with respect for individual autonomy and pluralism.
}
...
\begin{block}{Australia's Social Media Minimum Age Law}
Minimum age of \textbf{16} for social media accounts, in force \textbf{10 December 2025}. The duty falls on \emph{platforms} to take ``reasonable steps''---no penalty attaches to children or parents. Ten services are covered; we now have real data on how it went (next slide).
\end{block}
...
% ===== SLIDE 35b: Australia's Ban -- Early Evidence =====
\begin{frame}{Did It Work? Australia as a Natural Experiment}
Seven months of evidence on the world's first under-16 social media ban:

\vspace{0.2em}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{probox}[What happened]
\scriptsize
\begin{itemize}
    \item \textbf{5 million+} under-16 accounts removed or restricted
    \item Public support \textbf{rose} to roughly 77--79\% of adults
    \item The UK, Malaysia, Indonesia, Denmark and New Zealand moved to copy it
\end{itemize}
\end{probox}
\end{column}
\begin{column}{0.48\textwidth}
\begin{objectionbox}[What didn't]
\scriptsize
\begin{itemize}
    \item Under-16 usage fell only from \textbf{85.9\% to 81.5\%}
    \item \textbf{61\%} still had an active account; \textbf{70\%} called circumvention ``easy''
    \item \textbf{No fines} issued; no clear drop in cyberbullying complaints
\end{itemize}
\end{objectionbox}
\end{column}
\end{columns}
...
\vspace{0.2em}
{\scriptsize The workaround was \emph{social}, not technical---only 4--5\% used a VPN; most used a parent's ID or an existing login. Economists found teens need about \textbf{two of three peers to comply before they will}, and that those who \emph{did} comply reported more boredom and more trouble keeping up with friends.}

\begin{discussionbox}[Discussion]
Does a law count as working if it is popular but unenforced? If partial compliance mainly costs the students who obeyed, is that fair?
\end{discussionbox}
\end{frame}
% ===== YOU BE THE JUDGE =====
\begin{frame}[shrink=8]{You Be the Judge: Designing a Phone for a 14-Year-Old}
\begin{casestudybox}[The brief]
\small
You are on the product team shipping a phone aimed at \textbf{13--15 year olds}. The ship date is fixed and the hardware is done. You may remove exactly \textbf{three} software features. Nothing else is on the table.
\end{casestudybox}

\textbf{The menu}: infinite scroll \textbullet\ autoplay \textbullet\ streaks \textbullet\ push notifications \textbullet\ public like counts \textbullet\ beauty filters \textbullet\ the algorithmic feed \textbullet\ in-app purchases \textbullet\ disappearing messages

\begin{discussionbox}[Defend your three]
Not ``which is worst'' --- Aristotle's question. Each feature trains a \textbf{habit}. Name the habit yours would have built, and say what habit you are leaving in place because you ran out of cuts.
\end{discussionbox}
\end{frame}
...
\mode<article>{
The virtue ethics critique of contemporary technology faces important objections. Liberal critics like Isaiah Berlin warn that appealing to "positive liberty" or a particular conception of the good life risks paternalistic overreach that has historically justified tyranny. Nietzschean critics question whether we can reliably identify virtue versus vice, suggesting that historical "virtues" often merely reflected power structures or herd mentality. These objections are serious and demand careful responses concerning the distinction between protecting children and adults, the nature of autonomous choice under conditions of addiction and manipulation, the role of market failures in technology design, and the empirical basis for claims about human flourishing.
}
...
\small ``Positive liberty''---the freedom \textit{to} flourish---has historically justified tyranny in the name of people's ``true'' interests.
\end{quotebox}
\end{frame}
...
\footnotesize
\begin{description}
    \item[\textbf{Aristotle (384--322 BCE)}] argued that character is formed through \textit{habituation}---repeated practice that makes a disposition stable. The ethical question is not just what we do once, but what we do repeatedly.
    \item[\textbf{Eudaimonia}] is Aristotle's term for human flourishing---not pleasure, but the active exercise of one's highest capacities in a complete and characterful life.
    \item[\textbf{Virtue / \textit{arete}}] is a stable disposition to act, feel, and perceive in the right way; \textbf{vice} is its opposite. Both are cultivated---or corrupted---by the environments we inhabit, including digital ones.
    \item[\textbf{Shannon Vallor}] applies Aristotle to technology, arguing that social media and AI can habituate users into intellectual and moral vices that undermine flourishing.
    \item[\textbf{Negative liberty}] (Isaiah Berlin) is freedom \textit{from} interference; \textbf{positive liberty} is freedom \textit{to} flourish. The key objection to virtue-ethics regulation is that positive liberty arguments have historically justified paternalism and tyranny.
    \item[\textbf{Nietzsche}]'s challenge: who decides what counts as virtue? Historical ``virtues'' often reflected power structures; calling doom-scrolling a vice may simply reflect conservative bias.
\end{description}

\begin{discussionbox}[Discussion]
What practices in your own technological life cultivate virtue? Which cultivate vice?
\end{discussionbox}
\end{frame}
