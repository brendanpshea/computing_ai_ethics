...
\mode<article>{
This lecture provides a comprehensive introduction to artificial intelligence from philosophical and historical perspectives. The very definition of "AI" proves contested and unstable, with technologies once considered intelligent being reclassified as "mere computation" once understood---a pattern known as Tesler's Theorem. We examine various distinctions used to categorize AI systems (narrow vs. general, weak vs. strong, symbolic vs. connectionist) and their implications for ethical evaluation and policy development. The lecture traces AI's development from the Dartmouth Conference \parencite{dartmouth_proposal} through expert systems to modern machine learning and neural networks. We consider classical philosophical debates about machine intelligence from \textcite{turing_computing_machinery}, \textcite{searle_minds_brains}, and others, then examine how definitional choices shape contemporary discussions of AI capabilities, risks, and moral status.
}

% Slide 1: Title
\lecturetitleslide{images/fractal_07.png}


% ====================
% COLD OPEN
% ====================
% Sets up the Moving Goalposts slide, which supplies Tesler's Theorem.

\begin{frame}{Before We Begin: Which of These Is Artificial Intelligence?}
Hands up, yes or no, for each. \textbf{Commit} --- no ``it depends'' yet.

\begin{enumerate}
    \item A chess engine that beats every human alive
    \item The spam filter that cleaned out your inbox this morning
    \item A thermostat that learns your schedule
    \item The autocomplete finishing your sentences as you text
    \item ChatGPT
    \item The system choosing which video you see next
\end{enumerate}

\begin{discussionbox}[Commit before we go on]
Count your yes votes. Then say what \textbf{rule} you were applying --- there is one, even if you have not put it into words.
\end{discussionbox}
\end{frame}

\begin{frame}{The Answer: All of Them, and Then None of Them}
Every item on that list was called \textbf{artificial intelligence} by serious researchers when it was new. Most stopped being called AI at the moment they started working reliably.

\begin{alertblock}{The pattern you just reproduced}
Almost every class votes \textbf{no} on the chess engine and the spam filter and \textbf{yes} on ChatGPT. But Deep Blue beating Kasparov in 1997 was the AI story of the decade. What changed was not the chess engine. What changed was that we got used to it.
\end{alertblock}

This has a name --- \textbf{Tesler's Theorem}, which we will meet in a moment --- and it is a serious problem, not a joke. If ``AI'' names whatever we have not gotten used to yet, then every claim about what AI can or cannot do has a moving subject.
\end{frame}

% Slide 2: Learning Outcomes and Central Questions
\begin{frame}{Central Questions}
\begin{itemize}
    \item What \textit{is} artificial intelligence, and why is a stable definition so hard to pin down?
    ...
    \item How do definitional choices shape AI ethics, policy, and our understanding of our own intelligence?
\end{itemize}
...
\section{Defining AI: Definitional Challenges}

% Slide 3: Connections to Previous Lectures  
\begin{frame}{Connections to Previous Lectures}
\textbf{Lecture 1 (History)}: AI is the newest in our series of IT revolutions. It promises liberation (tool for productivity, creativity) and raises concerns about control (surveillance, job loss, autonomous weapons).

\vspace{0.2cm}
\textbf{Lecture 2 (Virtues)}: How do we develop intellectual virtues in interaction with AI systems that simulate understanding but may lack genuine comprehension?

\vspace{0.2cm}
\textbf{Lecture 3 (Free Speech)}: AI systems trained on vast data raise questions about whose speech is amplified, whose is silenced, and how definitions of ``intelligence'' affect content moderation.

\vspace{0.2cm}
\textbf{Lecture 4 (IP)}: Who owns AI-generated content? Can AI systems themselves be copyrighted? Training data and copyright battles.

\begin{discussionbox}[Discussion]
Which previous lecture's concerns appear most urgent in the context of AI?
\end{discussionbox}
\end{frame}

\mode<article>{
Defining artificial intelligence matters profoundly for how we understand its capabilities, assess its risks, and determine its moral status. The "moving goalposts problem" suggests that "AI" functions more as placeholder for unsolved problems than stable category, with successful AI repeatedly reclassified as "just" search algorithms, statistics, or optimization. Contemporary AI discourse employs multiple overlapping distinctions: narrow systems excelling at specific tasks versus hypothetical artificial general intelligence; weak AI that simulates intelligence versus strong AI that genuinely possesses minds; and symbolic rule-based approaches versus connectionist neural networks. Each distinction carries philosophical commitments about cognition, consciousness, and intelligence that shape both technical development and ethical evaluation.
}
...
\begin{tikzpicture}[scale=0.75, every node/.style={font=\small}]
    % Central node
    \node[draw, rounded corners, fill=boxblue!20, minimum width=2.5cm, minimum height=1cm] (def) at (0,0) {Definition of AI};
    
    % Branches
    \node[draw, rounded corners, fill=boxgreen!20, minimum width=2.2cm, minimum height=0.8cm] (cap) at (-4,-1.5) {Capabilities};
    \node[draw, rounded corners, fill=boxgold!20, minimum width=2.2cm, minimum height=0.8cm] (risk) at (0,-1.5) {Risks};
    \node[draw, rounded corners, fill=boxpurple!20, minimum width=2.2cm, minimum height=0.8cm] (moral) at (4,-1.5) {Moral Status};
...
\begin{discussionbox}[Discussion]
If a law regulates ``artificial intelligence, who benefits from the definition being \textbf{vague}? Who benefits from it being \textbf{narrow}?
\end{discussionbox}
\end{frame}
...
\scriptsize
Father of computer science. Cracked Enigma in WWII. Proposed the Turing Test \parencite{turing_computing_machinery} as a way to sidestep the question ``Can machines think?''
\end{figurebox}
\end{column}
\begin{column}{0.48\textwidth}
...
\scriptsize
Chinese Room argument \parencite{searle_minds_brains}. Syntax $\neq$ semantics; strong AI is false.
\end{figurebox}

\vspace{0.05em}
\begin{figurebox}[Daniel Dennett]
\scriptsize
Functionalist \parencite{dennett_consciousness}: if it functions like a mind, it \emph{is} a mind. Defends AI possibility.
\end{figurebox}

\vspace{0.05em}
\begin{figurebox}[David Chalmers]
\scriptsize
The ``hard problem'' \parencite{chalmers_conscious_mind}: why does processing \emph{feel} like something?
\end{figurebox}
\end{column}
\begin{column}{0.48\textwidth}
\begin{figurebox}[Stuart Russell]
\scriptsize
Standard AI textbook co-author \parencite{russell_norvig_ai}. AI = rational agency. AI safety advocate \parencite{russell_human_compatible}.
\end{figurebox}

\vspace{0.05em}
\begin{figurebox}[Luciano Floridi]
\scriptsize
AI achieves ``agency without intelligence''---effective action without understanding \parencite{floridi_ethics_ai}.
\end{figurebox}
\end{column}
\end{columns}
\end{frame}
...
\section{AI Through History: From Logic to Learning}
...
\vspace{0.2em}
\textbf{1950}: ``Computing Machinery and Intelligence'' \parencite{turing_computing_machinery}---Turing proposes the Imitation Game (Turing Test) and addresses objections to machine thought.
\end{column}
...
\begin{tikzpicture}[scale=0.6]
    ...
    % Head
    \draw[thick, fill=boxblue!30] (-0.2,-0.8) rectangle (0.6,-1.4);
...
% ===== IMAGE SLIDE: Alan Turing =====
\begin{frame}{Alan Turing: The Father of Computer Science}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{block}{Turing (1912--1954): A Tragic Genius}
\begin{itemize}
  \item Proved the theoretical limits of \textbf{computability} (1936) before computers existed
  \item Led \textbf{Bletchley Park} codebreakers during WWII; cracking Enigma shortened the war by years
  \item Proposed the \textbf{Turing Test} (1950) as a criterion for machine intelligence
  \item Prosecuted by the British government for \textbf{homosexuality} in 1952; died in 1954
  \item Awarded a \textbf{royal pardon} posthumously in 2013
\end{itemize}
\end{block}
\end{column}
\begin{column}{0.47\textwidth}
\begin{figure}
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{images/alan_turing.jpg}
\caption{\scriptsize Alan Turing, aged 16, c.\ 1928. Turing family. Public domain.}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\begin{casestudybox}[The Dartmouth Proposal \parencite{dartmouth_proposal}]
\scriptsize
...
\begin{tikzpicture}[scale=0.7, every node/.style={font=\scriptsize}]
    ...
    % Periods
    \draw[thick, boxgreen, fill=boxgreen!20] (0.5,0.2) rectangle (2.5,1.2);
    \node[text width=1.8cm, align=center] at (1.5,0.7) {Early optimism};
    
    \draw[thick, boxred, fill=boxred!20] (2.7,0.2) rectangle (4.7,-0.8);
    \node[text width=1.8cm, align=center] at (3.7,-0.3) {First Winter};
    
    \draw[thick, boxgreen, fill=boxgreen!20] (4.9,0.2) rectangle (7.2,1.2);
    \node[text width=2cm, align=center] at (6.05,0.7) {Expert Systems};
    
    \draw[thick, boxred, fill=boxred!20] (7.4,0.2) rectangle (9.4,-0.8);
    \node[text width=1.8cm, align=center] at (8.4,-0.3) {Second Winter};
    
    \draw[thick, boxgreen, fill=boxgreen!20] (9.6,0.2) rectangle (11.5,1.2);
...
\begin{tikzpicture}[scale=0.55]
    ...
    \foreach \y in {1,2,3,4} {
        \node[draw, circle, fill=boxblue!30, minimum size=0.5cm] (i\y) at (0,\y) {};
    }
    \node[below, font=\tiny] at (0,0.5) {Input};
    
    % Hidden layer 1
    \foreach \y in {1,2,3,4,5} {
        \node[draw, circle, fill=boxpurple!30, minimum size=0.5cm] (h1\y) at (2,\y-0.5) {};
    }
    
    % Hidden layer 2
    \foreach \y in {1,2,3,4,5} {
        \node[draw, circle, fill=boxpurple!30, minimum size=0.5cm] (h2\y) at (4,\y-0.5) {};
    }
    \node[below, font=\tiny] at (3,0) {Hidden layers};
    
    % Output layer
    \foreach \y in {2,3} {
        \node[draw, circle, fill=boxgreen!30, minimum size=0.5cm] (o\y) at (6,\y) {};
    }
...
\vspace{0.2em}
\textbf{Connection to privacy}: Recall Clearview AI (50+ billion photos scraped), predictive policing, and China's surveillance infrastructure.
...
\begin{itemize}
    ...
    \item \textbf{2020s}: Waymo robotaxis scale up; GM shuts down Cruise (2024)
\end{itemize}
...
\begin{tikzpicture}[scale=0.45, every node/.style={font=\tiny}]
    % Pipeline boxes
    \node[draw, rounded corners, fill=boxblue!20, minimum width=1.4cm, minimum height=0.6cm] (sense) at (0,3.6) {Sense};
    \node[draw, rounded corners, fill=boxblue!20, minimum width=1.4cm, minimum height=0.6cm] (perceive) at (0,2.5) {Perceive};
    \node[draw, rounded corners, fill=boxblue!20, minimum width=1.4cm, minimum height=0.6cm] (predict) at (0,1.4) {Predict};
    \node[draw, rounded corners, fill=boxblue!20, minimum width=1.4cm, minimum height=0.6cm] (plan) at (0,0.3) {Plan};
    \node[draw, rounded corners, fill=boxblue!20, minimum width=1.4cm, minimum height=0.6cm] (act) at (0,-0.8) {Act};
...
2023--24 & GPT-4, Claude, Gemini; multimodal models become standard \\
2025 & \textbf{Reasoning models}: capability starts scaling with thinking time at the moment of use, not just with training. Open-weight models close to within months of closed ones at a fraction of the cost \\
2026 & \textbf{Agents}: systems that operate software and act, rather than only answer. Release cycles compress to weeks; the most capable models ship in gated tiers \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{discussionbox}[Discussion]
You have used one of these systems. Describe one moment it seemed to \textbf{understand} you---and then say what else could explain that moment.
\end{discussionbox}
\end{frame}

\begin{frame}[shrink=8]{Two Changes Worth Naming}
\begin{block}{1. Capability stopped staying put}
In January 2025 a capable \textbf{open-weight} model---free to download and run---matched the leading closed systems at a small fraction of their cost. Nvidia lost roughly \textbf{\$600 billion} in market value in a single trading day, the largest one-day loss in market history. The assumption that frontier capability stays locked inside a few well-resourced labs no longer holds.
\end{block}

\vspace{0.2em}
\begin{alertblock}{2. Capability started constraining release}
In 2026, holding systems back \emph{because of what they could do} became routine. One lab kept a model to vetted partners over its ability to find and exploit software flaws; the White House asked another to release first to approved partners; a federal export-control order briefly forced a third model offline worldwide. By September, the leading labs were all shipping cyber-capable models in \textbf{two tiers}: a safeguarded public version and a gated one.
\end{alertblock}

\vspace{0.2em}
These two pull in opposite directions---one disperses capability, the other tries to contain it. Both will matter when we reach questions of governance and risk.

\begin{discussionbox}[Discussion]
One of these forces disperses capability and the other tries to contain it. \textbf{Which one is winning right now}---and what would you look at to tell?
\end{discussionbox}
\end{frame}

%% PART III: CLASSICAL DEBATES %%
\section{Can Machines Think? Classical Philosophical Debates}
...
\begin{tikzpicture}[scale=0.6]
    % Interrogator
    \node[draw, rounded corners, fill=boxgold!30, minimum width=2cm, minimum height=1cm] (int) at (0,3) {Interrogator};
    ...
    % Human and Machine
    \node[draw, rounded corners, fill=boxgreen!30, minimum width=1.5cm, minimum height=0.8cm] (human) at (-1.5,0) {Human};
    \node[draw, rounded corners, fill=boxblue!30, minimum width=1.5cm, minimum height=0.8cm] (machine) at (1.5,0) {Machine};
...
% ===== IMAGE SLIDE: Ada Lovelace =====
\begin{frame}{Ada Lovelace: The First Computer Programmer}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{block}{Ada Lovelace (1815--1852)}
\begin{itemize}
  \item Daughter of the poet Lord Byron; educated in mathematics by her mother
  \item Collaborated with Charles Babbage on the \textbf{Analytical Engine}
  \item Her \textbf{1843 notes} on Babbage's engine are considered the first computer algorithm
  \item Foresaw that machines could manipulate \textbf{symbols}---not just numbers
  \item The \textbf{Ada programming language} was named in her honor by the US Department of Defense in 1980
\end{itemize}
\end{block}
\end{column}
\begin{column}{0.47\textwidth}
\begin{figure}
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{images/ada_lovelace.jpg}
\caption{\scriptsize Ada Lovelace, daguerreotype by Antoine Claudet, c.\ 1843. Public domain.}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\begin{tikzpicture}[scale=0.55]
    ...
    % Rule book
    \draw[thick, fill=boxgold!20] (-1,-1.2) rectangle (1,-0.5);
...
\section{Contemporary Definitions and AI Approaches}
...
\begin{frame}{Russell \& Norvig---AI as Rational Agency \parencite{russell_norvig_ai}}
\begin{columns}[T]
\begin{column}{0.52\textwidth}
...
\begin{tikzpicture}[scale=0.55]
    % Agent box
    \node[draw, rounded corners, fill=boxblue!20, minimum width=2.5cm, minimum height=2cm] (agent) at (0,0) {};
    \node[font=\small] at (0,0.3) {Agent};
    \node[font=\tiny] at (0,-0.2) {(sensors, reasoning,};
    \node[font=\tiny] at (0,-0.5) {actuators)};
    
    % Environment
    \node[draw, rounded corners, fill=boxgreen!10, minimum width=2.5cm, minimum height=1.5cm] (env) at (0,-3) {Environment};
...
\small
The ``standard model'' builds machines that optimize fixed objectives. \textcite{russell_human_compatible} argues this is fundamentally dangerous.
\end{objectionbox}

\vspace{0.1em}
\begin{enumerate}
    \item We build AI systems to optimize objectives we specify.
    \item We can't fully specify what we value---human values are complex and tacit.
    \item A capable optimizer achieves its objective in ways that violate unstated values.
    \item Therefore, the more capable AI becomes, the more dangerous it is.
\end{enumerate}
...
\begin{frame}{Russell's Solution: Human Compatible AI \parencite{russell_human_compatible}}
...
\begin{frame}[shrink=8]{The Value Alignment Problem}
...
\textcite{russell_human_compatible}: This is \emph{the} central problem of AI. Capability without alignment is dangerous.
...
\begin{frame}{Floridi---AI as ``Agency Without Intelligence'' \parencite{floridi_ethics_ai}}
...
\end{frame}
% ===== YOU BE THE JUDGE =====
\begin{frame}[shrink=8]{You Be the Judge: Does It Understand?}
A system holds a forty-minute conversation about a novel it was never explicitly taught. It picks up a joke, corrects itself, and asks a question nobody asked it to ask.

\vspace{0.2em}
\textbf{Four benches. Each must rule, and each must state its test:}
\begin{itemize}
    \item \textbf{Turing}: is there any behavioral question left that would separate it from a person?
    \item \textbf{Searle}: is there anything here but symbol manipulation, and does the room understand Chinese?
    \item \textbf{Dennett}: if it does everything understanding does, what is the leftover thing you are asking about?
    \item \textbf{Floridi}: why assume it needs to understand at all in order to \textbf{act} on the world?
\end{itemize}

\begin{discussionbox}[The tiebreaker]
Which bench's test could you \textbf{actually run} on a system in front of you tomorrow? A criterion nobody can apply is not doing the work you think it is.
\end{discussionbox}
\end{frame}
...
\section{Why AI Definitions Matter: Implications and Ethics}
...
\begin{discussionbox}[Discussion]
Of everything we have covered today, which \textbf{one} disagreement would you most want settled before the next capable system ships?
\end{discussionbox}
\end{frame}
...
\footnotesize
\begin{description}
    \item[\textbf{Ada Lovelace (1815--1852)}] argued machines can only do what they are programmed to do---they cannot \textit{originate} anything. The earliest formal objection to machine intelligence.
    \item[\textbf{Alan Turing (1912--1954)}] proposed the \textit{Turing Test}: if a machine is indistinguishable from a human in conversation, call it intelligent---sidestepping whether machines ``really'' think.
    \item[\textbf{John Searle}] proposed the \textit{Chinese Room}: a system can pass the Turing Test by following rules without genuine understanding. Syntax is not semantics.
    \item[\textbf{Narrow AI}] excels at specific tasks; \textbf{general AI (AGI)} would match human flexibility across all domains. Most real systems are narrow.
    \item[\textbf{Weak AI}] simulates intelligent behavior without possessing a mind; \textbf{strong AI} would have genuine mental states---a distinction with implications for moral status.
    \item[\textbf{Symbolic AI (GOFAI)}] represents knowledge as explicit rules and logic; \textbf{connectionist AI} (neural networks) learns patterns from data---the dominant paradigm today.
    \item[\textbf{Tesler's Theorem / moving goalposts}]: ``AI is whatever hasn't been done yet.'' Once a capability is achieved, it gets reclassified as mere computation.
    \item[\textbf{Dartmouth Conference (1956)}] is where McCarthy, Minsky, and others named and launched ``artificial intelligence''---with optimistic timelines that proved wildly premature.
\end{description}

\begin{discussionbox}[Discussion]
How would \emph{you} define artificial intelligence---and does your definition commit you to any position on Lovelace, Turing, or Searle?
\end{discussionbox}
\end{frame}
