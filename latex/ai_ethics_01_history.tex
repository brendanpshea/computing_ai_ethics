% Lecture 1: The History of Information Technology and Ethics
% PHIL 1150: Computing and AI Ethics
...
% Title information
\title[History of Computing Ethics]{The History of Information Technology and Ethics}
\subtitle{Five Revolutions from Writing to the Web}
...
% TITLE SLIDE
% ====================
\lecturetitleslide{images/fractal_01.png}

% ====================
% COLD OPEN
% ====================
% Hook first, Central Questions second. Students commit to a guess before the
% reveal; almost everyone says phones or AI, and the answer is writing.

\begin{frame}{Before We Begin: Who Is Complaining, and About What?}
\begin{quotebox}[Someone, somewhere, about some new technology]
\small
``This invention will produce \textbf{forgetfulness} in the minds of those who learn to use it, because they will not practice their memory.

You offer your pupils the \textbf{appearance of wisdom}, not true wisdom, for they will read many things without instruction and will therefore \emph{seem} to know many things, when they are for the most part ignorant.''
\end{quotebox}

\begin{discussionbox}[Commit before we go on]
What technology is being attacked here? Who is speaking, and \textbf{what year}? Write down your guess --- no consulting your phone.
\end{discussionbox}
\end{frame}

\begin{frame}{The Answer: Socrates, About Writing, c.\ 370 BCE}
\textbf{Plato}, \textit{Phaedrus} 275a--b. Socrates relays King Thamus's verdict on the god Theuth, who has just invented \textbf{writing} and is proud of it.

\begin{alertblock}{Why start a course on AI ethics here}
Every charge we now level at phones, social media, and chatbots --- that they rot memory, counterfeit understanding, and substitute the \textbf{appearance} of knowing for the real thing --- was leveled first at the alphabet, and then at printing, the telegraph, radio, and television.
\end{alertblock}

That is not a reason to dismiss the charge. Sometimes the critics were right. The point is that we have \textbf{2{,}400 years of data} on how this argument goes, and we would be foolish not to look at it.
\end{frame}
...
\mode<article>{
This lecture examines the history of information technology ethics by tracing five major IT revolutions from ancient writing to the modern web. Each revolution promised liberation and democratization, yet also enabled new forms of control and manipulation. By understanding these historical patterns, we can better analyze contemporary ethical challenges in computing and artificial intelligence.
}
...
\begin{itemize}
    ...
    \item What historical lessons should we carry into our analysis of AI, IP, and surveillance?
\end{itemize}
...
\section{Introduction: Information Technology and Ethics}

% New slide: Connections to previous lectures  
\begin{frame}{Why Start with History?}
This course moves from \textbf{history} $\rightarrow$ \textbf{ethical frameworks} $\rightarrow$ \textbf{contemporary issues}.

Understanding IT history matters because:
\begin{itemize}
    \item \textbf{Patterns repeat}: The "concerns" about social media echo Orwell's and Arendt's warnings about TV
    \item \textbf{Power matters}: Technology serves whoever controls it---this lesson runs through every revolution
    \item \textbf{Speed of change}: Digital tech disrupted IP law, surveillance systems, and labor---understanding this helps us think critically about AI
\end{itemize}
...
\section{Revolution 1: Writing}

\mode<article>{
The invention of writing around 3200 BCE represented humanity's first major information technology revolution. By externalizing memory and enabling information storage beyond the human mind, writing fundamentally transformed human society. However, this transformation was not universally celebrated. The philosopher Socrates, as recorded by \textcite{plato_phaedrus}, articulated profound concerns about writing's impact on human cognition and wisdom---concerns that echo through subsequent debates about new technologies.
}
...
\begin{tikzpicture}[scale=0.7, transform shape]
  \node[draw, rounded corners, fill=boxblue!20, minimum width=3cm, minimum height=1cm] (oral) at (0,3) {Oral Culture};
  \node[draw, rounded corners, fill=boxblue!20, minimum width=3cm, minimum height=1cm] (written) at (0,0) {Written Culture};
  \draw[->, thick, boxblue] (oral) -- (written) node[midway, right, font=\scriptsize] {Writing invented};
...
\end{tikzpicture}
\caption{Transition from oral to written culture}
\end{figure}
\end{column}
\end{columns}
\end{frame}

% ===== IMAGE SLIDE: Cuneiform Tablet =====
\begin{frame}{Cuneiform: The World's First Writing System}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{block}{Key Facts}
\begin{itemize}
  \item Developed in \textbf{Mesopotamia} (modern Iraq), c.\ 3200 BCE
  \item Originally used for \textbf{bookkeeping}---tracking grain, cattle, and trade
  \item Impressed into \textbf{wet clay tablets} using a reed stylus
  \item Later adapted for laws, literature, and religion
\end{itemize}
\end{block}
\begin{exampleblock}{Legacy}
\small The oldest surviving literary work---the \textit{Epic of Gilgamesh}---was recorded on cuneiform tablets around 2100 BCE.
\end{exampleblock}
\end{column}
\begin{column}{0.47\textwidth}
\begin{figure}
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{images/cuneiform_tablet.jpg}
\caption{\scriptsize Cuneiform tablet, c.\ 2041--2040 BCE. Kirkor Minassian collection, Library of Congress. Public domain.}
\end{figure}
\end{column}
\end{columns}
\end{frame}

\begin{frame}[shrink=8]{Who Were Socrates and Plato?}
\begin{columns}
...
\begin{frame}[shrink=8]{Socrates and Plato's Worries About Writing}
\begin{columns}
\begin{column}{0.6\textwidth}
\begin{block}{The Irony}
\textbf{Socrates} never wrote anything down---yet we know his views through \textbf{Plato's} written dialogues. The myth itself is almost certainly \textbf{Plato's own invention}, not an Egyptian tradition he inherited: he wrote a story against writing, and it survives because he wrote it.
\end{block}
...
\begin{tikzpicture}[scale=0.9, transform shape]
  \node[draw, circle, fill=boxgold!30, minimum size=1.5cm, font=\scriptsize\bfseries] (theuth) at (0,2) {Theuth};
  \node[draw, circle, fill=boxblue!30, minimum size=1.5cm, font=\scriptsize\bfseries] (thamus) at (0,-1) {Thamus};
...
\end{tikzpicture}
\caption{The myth of Theuth and Thamus}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\vspace{0.5em}
\discussion{Is this argument \textbf{sound}---what might Socrates be missing? (The paradox: we only know his case against writing \textit{because Plato wrote it down}.)}
\end{frame}
...
\begin{tikzpicture}[scale=0.9, transform shape, 
    node distance=1.8cm,
    box/.style={draw, rounded corners, fill=boxblue!15, minimum width=3cm, minimum height=0.9cm, align=center, font=\small}]
  
  \node[box, fill=boxgold!30] (promise) {Promise of\\Liberation};
  ...
  \draw[->, thick, boxblue] (promise) -- (adoption);
  \draw[->, thick, boxblue] (adoption) -- (consequences);
  \draw[->, thick, boxblue] (consequences) -- (reflection);
  \draw[->, thick, boxblue] (reflection) -- (control);
...
\section{Revolution 2: The Printing Press}

\mode<article>{
Johannes Gutenberg's invention of movable type printing around 1440 marked the second great information technology revolution. By dramatically reducing the cost of book production, printing democratized access to knowledge and enabled rapid dissemination of ideas across Europe. Yet this same technology also facilitated the spread of propaganda, religious conflict, and witch-hunting manuals. The debate between Martin Luther and Erasmus exemplifies the tension between revolutionary and reformist visions of how new communication technologies should be deployed.
}
...
\begin{itemize}
  \item German goldsmith and inventor
...
% ===== IMAGE SLIDE: Gutenberg Bible =====
\begin{frame}{The Gutenberg Bible (1455)}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{block}{The First Major Printed Book}
\begin{itemize}
  \item Approximately 180 copies printed; about 49 survive
  \item Two folio volumes, 1,282 pages in total
  \item Quality rivaled the finest hand-copied manuscripts
  \item Each copy cost roughly \textbf{30 florins}---about 3 years' wages for a clerk
\end{itemize}
\end{block}
\begin{alertblock}{Why It Matters}
\small The Bible was the era's ultimate test case: could machine printing match the quality and authority of hand-copying?
\end{alertblock}
\end{column}
\begin{column}{0.47\textwidth}
\begin{figure}
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{images/gutenberg_bible.jpg}
\caption{\scriptsize Gutenberg Bible (Lenox Copy), New York Public Library. Photo: Kevin Eng. CC~BY-SA~2.0.}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\begin{tikzpicture}[scale=0.75, transform shape]
  \draw[thick, boxblue] (0,0) -- (5,0);
  \draw[thick, boxblue] (0,0.1) -- (0,-0.1);
  \draw[thick, boxblue] (5,0.1) -- (5,-0.1);
  \draw[thick, boxblue] (2.5,0.1) -- (2.5,-0.1);
  ...
  \node[below, font=\scriptsize\bfseries, text=boxblue] at (4.2,-0.3) {Erasmus};
  \fill[red!70!black] (0.8,0) circle (0.15);
  \fill[boxblue] (4.2,0) circle (0.15);
\end{tikzpicture}
...
\small
\textbf{Context:} Luther was among the first to grasp printing's power: his 95 Theses (1517) spread across Germany in weeks, and his German Bible became a bestseller. For Luther, this was providential.
...
\vspace{0.2em}
\hfill---attributed to Martin Luther, via Otto Clemen

\vspace{0.2em}
{\scriptsize \textbf{A note on sources}: this famous quote circulates widely but cannot be traced to Luther's own writings---it reaches us secondhand. (Sound familiar? We only know Socrates through Plato, too.)}
\end{exampleblock}

\vspace{0.2em}
\begin{block}{\faLightbulb\hspace{0.5em}The Tension}
\small Luther sees printing as \textbf{divinely ordained} for spreading truth. But the same press spreads what Luther would call \textbf{error}---and Luther's own late polemic ``On the Jews and Their Lies'' was later cited by Nazis to justify anti-Semitic policies.
\end{block}
\end{frame}
...
\vspace{0.5em}
\discussion{Both Luther and Erasmus have a point. Who gets to be the \textbf{gatekeeper}, who decides---and is there a middle ground?}
\end{frame}
...
\vspace{0.3em}
\discussion{Does the printing press bear \textbf{responsibility} for the witch trials---or is it just a tool? (The same press enabled the Scientific Revolution \emph{and} mass propaganda.)}
\end{frame}
...
\section{Revolution 3: Distance Communication}

\mode<article>{
The nineteenth century witnessed remarkable innovations in distance communication, particularly the telegraph and telephone. These technologies promised to unite humanity and even end war through better communication and understanding. However, they also enabled unprecedented imperial coordination and surveillance. The contrasting views of \textcite{mill1859liberty} and Karl Marx on these technologies reflect fundamentally different understandings of technology's relationship to power: Mill saw technology as a neutral tool enabling individual liberty, while Marx argued that technology inevitably serves whoever controls it \parencite{marx_capital}.
}
...
\begin{itemize}
  \item \textbf{Morse code}: Letters as electrical pulses (dots and dashes)
...
% ===== IMAGE SLIDE: Samuel Morse =====
\begin{frame}{Samuel F.B.\ Morse: Artist Turned Inventor}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{block}{Samuel Morse (1791--1872)}
\begin{itemize}
  \item Began his career as a \textbf{portrait painter}---not an engineer
  \item Inspired to build the telegraph after his wife died while he was away on commission
  \item Developed \textbf{Morse code} in the 1830s: letters as electrical dots and dashes
  \item First message (1844): ``\textit{What hath God wrought!}''
  \item Became one of the wealthiest men in America
\end{itemize}
\end{block}
\end{column}
\begin{column}{0.47\textwidth}
\begin{figure}
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{images/samuel_morse.jpg}
\caption{\scriptsize Samuel F.B.\ Morse, 1840. Archives of American Art, Smithsonian Institution. Public domain.}
\end{figure}
\end{column}
\end{columns}
\end{frame}

\begin{frame}[shrink=8]{Who Were Mill and Marx?}
\begin{columns}
\begin{column}{0.48\textwidth}
...
\vspace{0.2em}
{\scriptsize \textbf{A caution about how we're using them}: Mill and Marx never staged a debate about the telegraph, and neither wrote a systematic philosophy of communications technology. We are using them as \emph{representatives of two enduring positions} because they state those positions unusually clearly---not reporting an argument they actually had. Don't cite this slide as history.}
\end{frame}

% ===== IMAGE SLIDE: Karl Marx =====
\begin{frame}{Karl Marx: Technology, Power, and Ownership}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{block}{The Marxist Critique}
\begin{itemize}
  \item Technology is \textbf{not neutral}: it reinforces existing power structures
  \item Press and telegraph served \textbf{bourgeois} commercial interests
  \item Whoever \textbf{owns} the means of communication controls discourse
  \item ``The ruling ideas of each age have been the ideas of its ruling class''
\end{itemize}
\end{block}
\begin{discussionbox}[Discussion]
...
\begin{column}{0.47\textwidth}
\begin{figure}
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{images/karl_marx.jpg}
\caption{\scriptsize Karl Marx (1818--1883). Public domain.}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\vspace{0.5em}
\begin{tikzpicture}[scale=0.85, transform shape]
  \node[draw, rounded corners, fill=boxblue!20, minimum width=4cm] (mill) at (0,0) {\textbf{Mill}: Technology is neutral};
...
\begin{exampleblock}{\faQuoteLeft\hspace{0.5em}John Stuart Mill, \textit{On Liberty}, Chapter 2 \parencite{mill1859liberty}}
\small
...
\vspace{0.5em}
\discussion{Is Marx right that whoever \textbf{owns the technology} controls the message---or can you think of counterexamples?}
\end{frame}
...
\section{Revolution 4: Mass Media}

\mode<article>{
The rise of radio and television in the twentieth century created a fundamentally new communication paradigm: passive, one-to-many broadcasting controlled by a limited number of gatekeepers. George Orwell and Hannah Arendt, both witnesses to totalitarianism's rise, recognized mass media's potential to shape reality itself. \textcite{orwell_1984} warned of state monopolies on information that could make truth malleable, while Arendt analyzed how mass media combined with social atomization creates conditions favorable to totalitarian movements \parencite{arendt_origins}. These insights remain profoundly relevant in the digital age.
}
...
\begin{tikzpicture}[scale=0.6, transform shape]
  ...
  \foreach \angle in {30,60,90,120,150,210,240,270,300,330} {
    \node[draw, circle, fill=boxblue!20, minimum size=0.5cm, font=\tiny] (a\angle) at (\angle:2.5cm) {};
...
\begin{frame}[shrink=8]{Who Were Orwell and Arendt?}
\begin{columns}
\begin{column}{0.48\textwidth}
...
Both Orwell and Arendt personally witnessed the rise of totalitarianism, and saw many of their (highly intelligent!) contemporaries fall under its sway. One key question for both concerned the \textit{why} and \textit{how} of this phenomenon. Mass media played a central role.
\end{alertblock}
\end{frame}
...
\begin{exampleblock}{\faQuoteLeft\hspace{0.5em}George Orwell, \textit{Nineteen Eighty-Four}, Book 1, Chapter 7 \parencite{orwell_1984}}
\small
...
\begin{itemize}
  ...
  \item The precondition wasn't \textbf{ideology} but \textbf{loneliness}
  \item Breakdown of social bonds leaves individuals vulnerable
\end{itemize}
\end{block}

\vspace{0.3em}
\begin{exampleblock}{\faQuoteLeft\hspace{0.5em}Hannah Arendt, \textit{The Origins of Totalitarianism}, Ch.\ 13 \parencite{arendt_origins}}
\small
\textit{``What prepares men for totalitarian domination is the fact that \textbf{loneliness}, once a borderline experience suffered in marginal conditions like old age, has become an \textbf{everyday experience} of the ever-growing masses of our century.''}
\end{exampleblock}

\vspace{0.2em}
\begin{alertblock}{\faLightbulb\hspace{0.5em}The Insight}
Mass media doesn't just spread propaganda---it creates the \textbf{atomized individuals} who crave belonging enough to join movements promising meaning.
\end{alertblock}
\end{frame}

\begin{frame}{Arendt's Analysis of Propaganda}
\begin{argument}
  \item Mass society produces \textbf{atomized individuals} who feel superfluous.
  \item The lonely \textbf{crave coherent explanations} that give life meaning.
  \item Mass media delivers such explanations \textbf{to millions at once}.
  \item Totalitarian movements \textbf{exploit this} with all-encompassing ideologies.
  \item[\textbf{$\therefore$}] Mass media plus mass loneliness creates conditions \textbf{favorable to totalitarianism}.
\end{argument}
...
\section{Revolution 5: The World Wide Web}

\mode<article>{
The development of the World Wide Web in the 1990s and its evolution through Web 2.0 social media platforms represents our current information revolution. Initial utopian visions of democratization and disintermediation have given way to concerns about platform power, surveillance capitalism, and algorithmic manipulation. As with previous revolutions, we face the recurring question: who controls the technology, and for what ends? The debates surrounding artificial intelligence, content moderation, and digital privacy echo historical patterns while presenting genuinely novel challenges.
}
...
\vspace{0.3em}
\begin{tikzpicture}[scale=0.75, transform shape]
  \node[draw, rounded corners, fill=boxblue!20, minimum width=2.5cm, font=\scriptsize\bfseries] (web1) at (0,0) {Web 1.0};
  \node[draw, rounded corners, fill=boxgold!30, minimum width=2.5cm, font=\scriptsize\bfseries] (web2) at (5,0) {Web 2.0};
...
\section{Conclusion: Patterns and Lessons}

\mode<article>{
Across all five information technology revolutions, we observe recurring patterns. Each revolution promised liberation, democratization, and human progress. Each also enabled new forms of control, manipulation, and conflict. The optimists and pessimists were typically both partially correct. Most importantly, the ethical question is never simply about the technology itself, but about who controls it and toward what ends it is deployed. Understanding this history equips us to think more critically about contemporary technological change and our responsibilities as participants in ongoing digital transformation.
}
...
% ===== YOU BE THE JUDGE =====
\begin{frame}{You Be the Judge: The Phone Ban}
\begin{casestudybox}[The vote]
\small
Your school board is voting tonight on a \textbf{total ban on phones}, bell to bell, K--12. You are the deciding vote. Four consultants have filed briefs --- and every one of them has been dead for at least fifty years.
\end{casestudybox}

\textbf{Split into four groups.} Argue the ban from your thinker's actual position, in two minutes:
\begin{itemize}
    \item \textbf{Socrates}: the tool counterfeits the capacity it claims to extend
    \item \textbf{Erasmus}: the problem is not the press but the flood of bad print --- teach judgment, don't ban
    \item \textbf{Marx}: ask who owns the device and who profits from the attention
    \item \textbf{Arendt}: ask what happens to a public that shares no common world
\end{itemize}

\begin{discussionbox}[Then decide]
Which brief would \textbf{change its own author's mind} if you showed them the last twenty years of evidence?
\end{discussionbox}
\end{frame}
...
\footnotesize
\begin{description}
    \item[\textbf{Information technology}] is any technology for creating, storing, transmitting, or processing information---it is ancient, not uniquely modern.
    \item[\textbf{Theuth}] (Plato's \textit{Phaedrus}) is the Egyptian god who invented writing and promised it would aid memory; \textbf{Socrates} argued it would weaken genuine understanding---the first recorded technology debate.
    \item[\textbf{Luther}] embraced the printing press as a tool for mass reform; \textbf{Erasmus} feared an unfiltered flood of bad ideas. Both turned out to be right.
    \item[\textbf{Mill}] saw telegraph-era communication as aligned with free markets and progress; \textbf{Marx} analyzed how communication infrastructure reflects and reinforces ownership and class power.
    \item[\textbf{Orwell}] (\textit{Nineteen Eighty-Four}) and \textbf{Arendt} (\textit{Origins of Totalitarianism}) warned that mass media could become instruments of propaganda and totalitarian control.
    \item[\textbf{Disintermediation}] is the removal of gatekeepers promised by every IT revolution; \textbf{reintermediation} is the new gatekeepers---platforms, telecoms, states---that reliably replace them.
    \item[\textbf{Artificial scarcity}] is scarcity created by legal or technical means (copyright, DRM) where the underlying resource is naturally non-rivalrous.
\end{description}
